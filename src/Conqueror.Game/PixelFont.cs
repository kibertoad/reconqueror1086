using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;

namespace Conqueror.Game;

internal static class PixelFont
{
    private static readonly Dictionary<char, string[]> Glyphs = new()
    {
        ['A']=["01110","10001","10001","11111","10001","10001","10001"], ['B']=["11110","10001","10001","11110","10001","10001","11110"],
        ['C']=["01111","10000","10000","10000","10000","10000","01111"], ['D']=["11110","10001","10001","10001","10001","10001","11110"],
        ['E']=["11111","10000","10000","11110","10000","10000","11111"], ['F']=["11111","10000","10000","11110","10000","10000","10000"],
        ['G']=["01111","10000","10000","10111","10001","10001","01111"], ['H']=["10001","10001","10001","11111","10001","10001","10001"],
        ['I']=["11111","00100","00100","00100","00100","00100","11111"], ['J']=["00111","00010","00010","00010","10010","10010","01100"],
        ['K']=["10001","10010","10100","11000","10100","10010","10001"], ['L']=["10000","10000","10000","10000","10000","10000","11111"],
        ['M']=["10001","11011","10101","10101","10001","10001","10001"], ['N']=["10001","11001","10101","10011","10001","10001","10001"],
        ['O']=["01110","10001","10001","10001","10001","10001","01110"], ['P']=["11110","10001","10001","11110","10000","10000","10000"],
        ['Q']=["01110","10001","10001","10001","10101","10010","01101"], ['R']=["11110","10001","10001","11110","10100","10010","10001"],
        ['S']=["01111","10000","10000","01110","00001","00001","11110"], ['T']=["11111","00100","00100","00100","00100","00100","00100"],
        ['U']=["10001","10001","10001","10001","10001","10001","01110"], ['V']=["10001","10001","10001","10001","10001","01010","00100"],
        ['W']=["10001","10001","10001","10101","10101","10101","01010"], ['X']=["10001","10001","01010","00100","01010","10001","10001"],
        ['Y']=["10001","10001","01010","00100","00100","00100","00100"], ['Z']=["11111","00001","00010","00100","01000","10000","11111"],
        ['0']=["01110","10001","10011","10101","11001","10001","01110"], ['1']=["00100","01100","00100","00100","00100","00100","01110"],
        ['2']=["01110","10001","00001","00010","00100","01000","11111"], ['3']=["11110","00001","00001","01110","00001","00001","11110"],
        ['4']=["00010","00110","01010","10010","11111","00010","00010"], ['5']=["11111","10000","10000","11110","00001","00001","11110"],
        ['6']=["01110","10000","10000","11110","10001","10001","01110"], ['7']=["11111","00001","00010","00100","01000","01000","01000"],
        ['8']=["01110","10001","10001","01110","10001","10001","01110"], ['9']=["01110","10001","10001","01111","00001","00001","01110"],
        ['.']=["00000","00000","00000","00000","00000","00110","00110"], [':']=["00000","00110","00110","00000","00110","00110","00000"],
        ['-']=["00000","00000","00000","11111","00000","00000","00000"], ['/']=["00001","00010","00010","00100","01000","01000","10000"],
        ['%']=["11001","11010","00010","00100","01000","01011","10011"], ['+']=["00000","00100","00100","11111","00100","00100","00000"],
        ['!']=["00100","00100","00100","00100","00100","00000","00100"], ['?']=["01110","10001","00001","00010","00100","00000","00100"],
        [',']=["00000","00000","00000","00000","00110","00110","00100"], ['\'']=["00100","00100","00010","00000","00000","00000","00000"],
        ['"']=["01010","01010","01010","00000","00000","00000","00000"], ['(']=["00010","00100","01000","01000","01000","00100","00010"],
        [')']=["01000","00100","00010","00010","00010","00100","01000"], ['_']=["00000","00000","00000","00000","00000","00000","11111"],
        ['=']=["00000","11111","00000","11111","00000","00000","00000"], [';']=["00000","00110","00110","00000","00110","00110","00100"],
        ['&']=["01100","10010","10100","01000","10101","10010","01101"], ['#']=["01010","11111","01010","01010","11111","01010","00000"],
        ['[']=["01110","01000","01000","01000","01000","01000","01110"], [']']=["01110","00010","00010","00010","00010","00010","01110"]
    };

    public static void Draw(SpriteBatch batch, Texture2D pixel, string text, Vector2 pos, Color color,
        int scale = 3, int wrap = 0, OriginalUiFont? original = null)
    {
        if (original is not null)
        {
            original.Draw(batch, text, pos, color, scale, wrap);
            return;
        }
        if (scale <= 0) throw new ArgumentOutOfRangeException(nameof(scale));
        var renderedText = wrap > 0 ? PixelTextLayout.Wrap(text, Math.Max(1, wrap / (6 * scale))) : text;
        var x = (int)pos.X; var y = (int)pos.Y; var origin = x;
        foreach (var raw in renderedText.ToUpperInvariant())
        {
            var normalized = raw switch
            {
                '‘' or '’' => '\'',
                '“' or '”' => '"',
                '–' or '—' => '-',
                _ => raw
            };
            var c = normalized == '\n' || char.IsWhiteSpace(normalized)
                ? normalized == '\n' ? '\n' : ' '
                : Glyphs.ContainsKey(normalized) ? normalized : '?';
            if (c == '\n') { x = origin; y += 9 * scale; continue; }
            if (c != ' ')
            {
                var glyph = Glyphs[c];
                for (var row = 0; row < 7; row++) for (var col = 0; col < 5; col++)
                    if (glyph[row][col] == '1') batch.Draw(pixel, new Rectangle(x + col * scale, y + row * scale, scale, scale), color);
            }
            x += 6 * scale;
        }
    }
}
