...
Check(InnPresentationDefinitions.Fallback.Patrons.Select(x => x.Name).SequenceEqual(["Frederick", "Gerard", "Barkeep", "Otto", "Hugh", "Gilbert", "Nellie", "Richard", "Ivo", "Albert"]), "executable-ordered inn patron labels are definition driven");
Check(InnPresentationDefinitions.Fallback.Patrons.Select(x => x.ConversationRootNodeId).SequenceEqual([1900, 1100, 3200, 3500, 1600, 1400, 3000, 1698, 3300, 3600]), "inn patron conversation roots are definition driven");
Check(ConversationPresentationDefinitions.ResponseBounds(0) == new UiBounds(41, 302, 558, 35) && ConversationPresentationDefinitions.ResponseBounds(4) == new UiBounds(41, 442, 558, 35), "conversation response rows match the original renderer");
