...
public sealed partial class ResourceAndDefinitionTests
{
    ...
    public void ImportedSceneLayoutPreservesInteractiveTilesSpawnsAndFacing()
    {
        ...
        Assert.Equal((7, 12, 0), EnemyStats(Assert.Single(siege.Enemies, enemy => !enemy.Champion)));
        Assert.Equal((10, 20, 0), EnemyStats(Assert.Single(siege.Enemies, enemy => enemy.Champion)));

        static (int? Armor, int Health, int? Row) EnemyStats(SiegeEnemy enemy) => (enemy.OriginalArmor, enemy.Health, enemy.OriginalCombatRow);
    }
