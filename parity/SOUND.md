...
| `RULE-SOUND-002` | Playing a sample | supported | partial | None | `DEV-SOUND-001` | supported | Only the shared click (sample 0 of `gameopts.666`) is bound, played on every click while effects are on. The host mixer has no ten-voice limit, draws nothing from the game's generator, uses the effects volume, and plays the 11,050 Hz samples at their own rate. |
