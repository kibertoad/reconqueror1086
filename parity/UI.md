...
| `FMT-UI-003` | Exterior catalogs, VILLAGE.DAT and TVILLAGE.DAT | supported | complete | None | None | implemented | `VillageSceneCatalogDecoder` repairs the three malformed rows, so the record 37 lender keeps the numbers its line gives (BUG-UI-001 is not reproduced). It keeps the labels, which the original discards, and rejects any other malformed row. |
