...
| `RULE-TALK-003` | Script functions and conversation variables | supported | partial | None | None | supported | The seven selectors match; unmapped selectors keep values of their own, attribute writes clamp to 0 to 20 (wealth to 0 or more), an INT_MIN assignment to an attribute is refused, an add is not checked for INT_MIN, and variable 190 is out of range (BUG-TALK-001). |
