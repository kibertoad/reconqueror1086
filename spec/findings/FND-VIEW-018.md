---
id: FND-VIEW-018
title: Scene textures are headerless TEX resources named with their index, width and height, one byte per pixel
status: recorded
builds: [BLD-GOG-EN]
superseded_by: []
...
The 99 `.RES` and `.LOW` archives in `CD:CONQUER` hold 12,982 resources whose names are
`TEXnnn width height`: `TEX` and a three-digit decimal index, then the width and the height as
positive decimal numbers, separated by white space. The size of every one is exactly
`width * height` bytes. In paired archives the `.LOW` textures are mostly 64 pixels wide where
the `.RES` ones are 128.

## Interpretation

A texture is `width * height` palette indices with no header and no row padding; the `.LOW`
archives hold the same scenes at lower texture resolution.
...
List the entries of any scene archive and compare each `TEX` entry's size with the product of
the two numbers in its name.
