...
`SiegeActorMovement` retains the tick interval, coordinate delta, descriptor flags, and surface stride. `SiegeLayout` carries a separate movement-block plane derived from authored behavior bit 2 rather than semantic visual labels; imported actor starts use their state-target block beneath them, and every object stage carries its own bit so immediate state replacement updates collision at the same time. `SiegeSession.AdvanceRetainerMovement` applies the direct-handler flag rewrite, recovered effect-boundary re-aim, cardinal rotations, strict per-tick deadlines with bounded catch-up, blocker-band rejection, retained-offset early effect completion/retry, open-path completion boundary, signed 8.8 remainder, and three walk-surface groups to selected mode-12 travel, acquired-target Attack, and Follow. `SiegeViewProjection.ProjectActors` adds those offsets divided by 256 so rendering and pointer picking share the moving point. Focused tests cover strict equality, first and sustained transition formulas, four cardinal rotations, clockwise diagonal ties, authored blocker masks, blocked-effect retry after a state change, effect-boundary completion, update-frequency independence, imported offsets/profile fields, sub-cell projection, explicit hostile identity, and Attack/Follow movement. The `0x40` corner/turn branches, multi-actor convergence, acquisition thresholds, no-target mode 6, Retreat mode 10, and full actor-wide cadence remain **Provisional**.
