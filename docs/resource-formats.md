...
The foreground renderer at `0x5529B`-`0x55448` selects stored offset 2 on a melee weapon's approach, offset 1 near contact, and offset 0 on return; crossbow rows retain offset 2. Combat-row column 3 is not a frame interval: `0x54FB8`-`0x54FC8` computes signed foreground velocity as `(screen displacement << 9) / column3`, and `0x54FEC`-`0x5501E` integrates it with elapsed milliseconds before pose selection. All 25 divisors (380-1,000) are now mapped in `OriginalWeaponCombat`. The pose order, data field, integer formula, and elapsed-time integration are **Confirmed**; exact randomized screen-space trajectory and modern rendering adaptation remain **Provisional**, while the former universal 70 ms frame timer is **Disproved**.
