...
The pointer routine at `0x470A8` is a projected candidate resolver, not a floor-plane formula. Switch table `0x34A0C` excludes kind 0; `0x44F7C` collects up to 32 block contacts; and `0x470A8` projects their decoded lower/upper elevations before `0x444E8` alpha sampling. This **Disproves** both the earlier speculative 60-degree half-cell implementation and the later empty-floor integer bridge. Exact multi-candidate ordering, diagonal shapes, cropped wrapping, and corner order remain **Provisional**.
