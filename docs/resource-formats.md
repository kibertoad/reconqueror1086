...
Runtime scene animations are not governed by a format-wide fixed frame duration. `0x4C7F4` constructs up to 64 transient `0x40`-byte scheduler records and stores a caller-supplied millisecond interval at record offset `+0x14`; `0x530D0` advances active records against their own deadlines. The resource/template field that supplies each interval is not mapped yet, so no duration should be inferred from adjacency or animation-frame count. The current clean-room door/enemy timing constants remain **Provisional**, and treating them as a recovered universal cadence is **Disproved**.
