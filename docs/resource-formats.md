...
`SiegeViewProjection.GroundCell` provides the current compatibility bridge: below-horizon pointer coordinates intersect a half-cell-height floor plane along the same horizontal view angle, and the existing DDA wall depth rejects a destination hidden behind solid geometry. `SiegeSession.CommandSelectedRetainersTo` stores the destination only on selected living retainers, clears selection and explicit hostile targeting, moves toward it until the exact cell matches, then resumes the retainer's prior public command. This activates the confirmed mode-12 semantics. Camera-height/floor-plane equivalence, fixed-point rounding, route selection, collision resolution when several retainers share a target, and one-cell cardinal motion remain **Provisional**. Tests lock centered ground mapping, horizon rejection, wall occlusion, selected-only dispatch, destination completion, and prior-command resumption.

Actor movement uses a second block selector. The visual state-completion selector is signed word `+0x2E`; mode handlers such as `0x4FFBC` instead load the dword at block `+0x44`, arithmetic-shift it by 16, and use signed word `+0x46` as the movement-effect index. `DynamixSceneBlock.MovementEffectDefinitionIndex` exposes and bounds-checks it separately. The inspector's ignored `--scene-blocks` report correlates both selectors with the chosen descriptor fields. Across all 144 placed actor bases and 1,002 placements in the 42 owned `MELEE*`/`DEFEND*` scene pairs, 134 bases use movement descriptor 7 and ten use descriptor 10; both resolve to exactly `FrameCount=3`, `IntervalMilliseconds=200`, `Flags=0x142`, `MapXDeltaPerTick=64`, and `MapYDeltaPerTick=0`.

Handler `0x4FF53` quantizes heading and passes that descriptor to constructor `0x4C7F4` with flags rewritten as `(flags & 0xA7) | 0x10`. Scheduler `0x530D0` uses strict `elapsed > interval`, rotates local `(64,0)` by the actor heading through `0x44740`, and applies the 8.8 substep. Its collision path treats the central band as `-0x59..0x59`, tests the neighboring map cell outside it, and recenters after crossing `+/-0x80`; the third 64-unit tick therefore enters the adjacent cell. The corroborated nominal cell formula is `FrameCount * IntervalMilliseconds = 3 * 200 = 600 ms`, while equality waits for the next scheduler call. `SiegeActorMovement` retains all five profile values, and selected destination orders advance against this strict elapsed-time deadline rather than player input. Current route ordering, intermediate sub-cell draw position, collision deflection, and catch-up policy are not claimed identical and remain **Provisional**.
