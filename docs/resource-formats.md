...
`Scenario` offsets 40, 44, 48, and 52 are respectively the color-mapping enabled flag, map count, distance shift, and blend target; block offset 12 is a signed per-block color-map offset. The executable selects `clamp((depth8_8 >> (DistanceShift - 2)) - blockOffset, 0, MapCount - 1)`, with repeated render paths at LE virtual addresses `0x4581F`-`0x45854`, `0x45D1B`-`0x45D4F`, and `0x45F3E`-`0x45F70`. Every owned Scenario enables 32 maps. The melee family uses shift 10 and blend target 20, so cell distance selects `Pal0` through `Pal31` directly before applying the block offset. The renderer now follows this rule and retains each encountered texture/map result for the active scene; transparency follows the original source index rather than a remapped black index. The executable also regenerates the first family toward the Scenario blend target at `0x47914`-`0x47A84`; reproducing that palette-dependent generation instead of using the stored family remains **Incomplete**.
