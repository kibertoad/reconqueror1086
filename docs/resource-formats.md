...
The structure, sizes, column ordering, fixed-point viewer position, named block roles, shape texture slots, and distinct kind-4 primary-image slot are **Confirmed for the hashed release** by cross-checking all 12,784 block records and locating Viewer positions and coherent images in multiple scene variants. `SKIRMISH.PAL` and the contacted cardinal surface slot now drive runtime wall columns, while kind-4 actors use only their primary billboard texture. The palette association is confirmed by executable generation and population-wide byte equality. The intersection routine at `0x44CC0`-`0x44F61` emits masks `0x100`, `0x200`, `0x400`, and `0x800` for the north, east, south, and west faces; the render selector at `0x46739`-`0x467F7` maps them to block offsets 44, 48, 52, and 56 respectively. Palette association and cardinal slot ordering are therefore **Confirmed**. Adjacent door-state and actor-animation run structure is described above and below. Placed kind-4 behavior-35 blocks also form adjacent named state sequences for trees, barrels, stools, pots, and candelabras; their ordering and transition from collision to pickup/floor are **Corroborated** across the owned scenes. The meanings of the remaining block fields, exact heading orientation, collision variants, and exact destructible event/damage timing remain **Provisional**.
