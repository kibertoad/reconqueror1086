...
Runtime scene animations are not governed by a format-wide fixed frame duration. Scene loading at `0x50AAC`-`0x50ACC` uses Scenario offset `0x1C` as a record count and reads exactly `count * 0x40` bytes from the archive entry named `SFXDEFS`. Within each descriptor, `+0x0C` is the frame count, `+0x14` is the interval in milliseconds, `+0x18` holds scheduler flags, and `+0x28` is either `-1` or a scene-block selector. The signed word at scene-block offset `0x2E`—the high word of the 32-bit field at `0x2C`—selects a descriptor in constructor callers such as `0x502F0`-`0x50345`; ranged combat rows 23 and 24 double its interval at `0x5032C`-`0x50332`. Constructor `0x4C7F4` creates up to 64 transient `0x40`-byte records, and scheduler `0x530D0` advances them against independent deadlines. `DynamixSceneEffectDecoder` retains all 16 fields without inventing names for those still unknown; `DynamixSceneDecoder` validates descriptor counts, intervals, map-block references, and block selectors. The owned-disc inspection validates these relationships across all 99 scene containers, with targeted malformed-input coverage in `ResourceAndDefinitionTests.SceneEffects.cs`. Exact state/event binding and the remaining descriptor fields are still open. Current clean-room door/enemy timing constants remain **Provisional**, and treating them as a recovered universal cadence is **Disproved**.
