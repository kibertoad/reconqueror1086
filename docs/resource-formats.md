...
Empty-ground mode 12 is now mapped. At `0x555BE`-`0x5562E`, each selected actor receives the raycaster contact x/y (shifted from 8.8 fixed point to integer cells) in `+0x28/+0x2C`, current mode is reset, requested mode becomes 12, transition helper `0x4E5F0` is called, and selection bit 0 is cleared. Acquisition branch `0x4FD7B` compares both current integer map coordinates with those stored targets and exits the mode only when both match. Handler `0x4FF53` requires no active effect, calculates heading from `target - current` through `0x445C4`, applies `(heading + 0x20) & 0xC0`, clears target actor `+0x24`, writes the actor block into the current map cell, copies its base state, and constructs a coordinate-changing effect through `0x4C7F4`. The piecewise integer heading calculation produces diagonal values `0x20/0x60/0xA0/0xE0`, so the added half-quadrant makes exact ties select the clockwise cardinal. Target-coordinate meaning, selected-only dispatch, mode number, cardinal quantization, target-actor clearing, and exact-cell completion are **Confirmed**.

`SiegeViewProjection.GroundCell` provides the empty-floor bridge using executable-backed integer projection. Viewer initialization `0x5421F`-`0x542E4` supplies elevation `0x80`, integer horizon `height / 2`, and viewport width. `0x470A8` forms horizontal lateral basis `((0x400000 / width) * (x - width / 2)) >> 8` beside forward basis `0x4000`; dividing the camera height by the below-horizon row yields the 8.8 forward distance. Traversal helper `0x45158` normalizes its major axis to signed `0x100` and walks at most `0x40` map steps. The runtime preserves those formulas and bound and rejects an intervening wall. `SiegeSession.CommandSelectedRetainersTo` stores the destination only on selected living retainers, clears selection and explicit hostile targeting, re-aims through the recovered cardinal rule at each effect boundary, and resumes the prior public command only when an effect completes in the exact target cell. Formula and bound are **Confirmed**; the runtime bridge is **Corroborated** while candidate-helper surfaces, wrapped 128-cell source-map lookup, exact corner treatment, and behavior when several retainers share a target remain **Provisional**. Tests lock center/edge mapping, horizon and 64-step rejection, wall occlusion, selected-only dispatch, diagonal quantization, effect-boundary completion, and prior-command resumption.
...
The pointer routine at `0x470A8` is a projected candidate resolver, not merely a standalone floor-plane formula. It builds the fixed-point camera vector described above through `0x446BC`/`0x4472C`/`0x447C4`, asks `0x45158` to traverse no more than `0x40` wrapped map steps and populate candidate arrays, transforms kind-4 block offsets, and calls pixel-mask test `0x444E8` before writing its multiple outputs. This **Confirms** the camera formula, traversal bound, and visible-billboard participation and **Disproves** the earlier speculative 60-degree half-cell implementation. Full empty-ground equivalence still requires the candidate helper's output surface, wrapping, and corner contract to be closed and remains **Provisional**.
