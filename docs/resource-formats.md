...
The structure, sizes, fixed-point viewer position, and named block roles are **Confirmed for the hashed release** by cross-checking all 12,784 combat-scene block records and locating Viewer positions and coherent images in multiple scene variants. `SKIRMISH.PAL` is the palette of the combat scenes; the association is confirmed by executable generation and population-wide byte equality.

Renderer `0x46E3B`-`0x46F23` calculates `sector = ((((relative heading + 0x80 / divisions) & 0xff) * divisions) >> 8)`, where `divisions` is block offset `+0x34`, mirrors the source column and folds the sector when behavior bit 2 is set and the sector exceeds `divisions / 2`, then selects block offset `+0x2C`'s low-word base surface plus that sector.
...
Every owned Scenario enables 32 colour maps. The melee family uses distance shift 10 and blend target 20, so cell distance selects `Pal0` through `Pal31` directly before applying the block offset.
