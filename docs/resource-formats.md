...
| `+0x34` | 4 | movement mode; 1 is direct property movement, 2 is waypoint-buffer movement, and 3 pursues another live strategic record |
...
Routed updater `0x4A300` maps a one-rounded-direction lookahead through `0x629B0`/`0x3E72C`, then reads its unsigned terrain kind through the byte lookup at object-2 `+0xAF78`. Kind 9 is the deactivation case. The current profile at `+0xB610` indexes four pointers at `+0xB704`: profiles 0, 1, and 3 all point to the 30-float table at `+0xB614`; profile 2 points to the distinct 30-float table at `+0xB68C`. The primary values are `1,.2,1,.5,1,.5,.8,.8,1,5,.3,.6,.8,.8,2,2,2,1,.2,.5,.5,.5,.5,3,1,1,.5,.5,.5,.2`; the profile-2 values are `.8,.2,.8,.5,.8,.5,.6,.6,.8,5,.2,.4,.5,.5,1.5,1.5,1.5,.8,.1,.4,.4,.4,.4,2,1,1,.5,.5,.5,.6`. Each displacement is normalized direction times the selected float times elapsed milliseconds, stored at movement `+0x64/+0x68`. Only the absolute unrounded horizontal product is compared with object-2 double `+0x7389 = 50.0`; strict `< 50.0` applies both components, while equality or greater holds position without deactivating. The prior stored components are independently rounded and validated against inclusive `-50..50` on the next call. These tables, pointer sharing, kind-9 outcome, arithmetic order, and asymmetric strict application gate are **Confirmed** by `0x4A528`-`0x4A607` and the derived `--strategic-terrain-table` report. `TerrainSpeed` and `CalculateRoutedStep` expose them with focused tests. Terrain-kind names, the selector's higher-level meaning, exact alternate-route identities, live decoded-path consumption, and save migration remain **Provisional**.
