...
The 15 two-digit `MELEE00`–`MELEE24` archives belong to the tournament name builder, not to a one-per-castle campaign sequence. The practice callback selects inclusively among the three distinct unsuffixed `MELEE0`–`MELEE2` archives. Literal `MELEE0.RES` is loaded separately for the common campaign path; that binding is **Corroborated** pending the final indirect callback.
...
The foreground renderer at `0x5529B`-`0x55448` selects stored offset 2 on a melee weapon's approach, offset 1 near contact, and offset 0 on return; crossbow rows retain offset 2. The runtime preserves that confirmed pose order, while its timing interval remains **Provisional**.
