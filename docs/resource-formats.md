...
`SiegeViewProjection.GroundCell` provides the current compatibility bridge: below-horizon pointer coordinates intersect a half-cell-height floor plane along the same horizontal view angle, and the existing DDA wall depth rejects a destination hidden behind solid geometry. `SiegeSession.CommandSelectedRetainersTo` stores the destination only on selected living retainers, clears selection and explicit hostile targeting, re-aims through the recovered cardinal rule at each effect boundary, and resumes the prior public command only when an effect completes in the exact target cell. This activates the confirmed mode-12 semantics. Camera-height/floor-plane equivalence, fixed-point ray rounding, collision corner interactions, and behavior when several retainers share a target remain **Provisional**. Tests lock centered ground mapping, horizon rejection, wall occlusion, selected-only dispatch, diagonal quantization, effect-boundary destination completion, and prior-command resumption.
...
The pointer routine at `0x470A8` is a projected candidate resolver, not a standalone floor-plane formula. It builds fixed-point view vectors through `0x446BC`/`0x4472C`/`0x447C4`, asks `0x45158` to traverse no more than `0x40` wrapped map steps and populate candidate arrays, transforms kind-4 block offsets, and calls pixel-mask test `0x444E8` before writing its multiple outputs. This **Confirms** traversal and visible-billboard participation and **Disproves** the earlier idea that `0x470A8` alone can justify the runtime's half-cell plane. Exact empty-ground return-coordinate arithmetic still requires the candidate helper/output contract to be closed and remains **Provisional**.
