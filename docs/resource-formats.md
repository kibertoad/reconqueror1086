...
Loader `0x5185F`-`0x51875` writes actor-record coordinates `(cellX << 8) + 0x80` and `(cellY << 8) + 0x80`. Handler `0x4FF53` quantizes heading and passes the descriptor to constructor `0x4C7F4` with flags rewritten as `(flags & 0xA7) | 0x10`. Scheduler `0x530D0` uses strict `elapsed > interval`, rotates local `(64,0)` by actor heading through `0x44740`, and adds it to cloned-block `+0x24/+0x28`. For either axis, with signed delta `d` and retained offset `o`, the core update is `n = o + d`; outside the strict central band `-0x59..0x59` it tests the neighboring block's collision bit 2, and only when `n < -0x80` or `n > 0x80` does it change the map cell and wrap `n` by `+0x100` or `-0x100`. Equality at `+/-0x80` stays in the current cell. Starting from zero, the official delta yields offsets 64, 128, then 192; the third tick enters the next cell with offset -64. Consequently `FrameCount * IntervalMilliseconds = 3 * 200 = 600 ms` is the movement-effect duration and the first cell transition, while retained remainder makes sustained straight motion one cell per four ticks, `4 * 200 = 800 ms`. The former universal 600 ms cell formula is **Disproved**.

`SiegeActorMovement` retains the tick interval, coordinate delta, flags, and surface stride. `SiegeSession.AdvanceRetainerMovement` applies cardinal rotations and strict per-tick deadlines with bounded catch-up, preserves signed 8.8 remainder across cells, and steps the three walk-surface groups; `SiegeViewProjection.ProjectActors` adds those offsets divided by 256 so rendering and pointer picking share the moving point. Focused tests cover strict equality, the first and sustained transition formulas, four cardinal rotations, update-frequency independence, imported offsets/profile fields, and sub-cell projection. Current route ordering, post-rejection collision deflection, multi-actor routing, and movement outside selected mode-12 destination travel are not claimed identical and remain **Provisional**.
