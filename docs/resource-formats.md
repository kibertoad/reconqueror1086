...
At `0x54F98`-`0x54FE5`, both axis velocities are `(target - outer) << 9 / combat-row-column-3`; `0x54FEC`-`0x5501E` integrates those signed 8.8 values with elapsed milliseconds. Contact at `0x550F2`-`0x55140` reverses horizontal velocity and reverses/halves vertical velocity for melee rows. Renderer `0x5529B`-`0x55448` selects stored offset 2 on approach, offset 1 when horizontal target separation is less than `W/4`, and offset 0 on return, while rows 23-24 retain offset 2; its mirrored and ordinary blitters anchor on opposite sides of the stored x coordinate. All 25 divisors (380-1,000), coordinate branches, random spans, fixed offsets, velocity formula, contact reversal, pose threshold, and mirror rule are **Confirmed**. `SiegeForegroundTrajectory` implements the three melee families with retained 8.8 subpixels, whole-millisecond updates, the original 166 ms update cap, and render-independent pose selection; `ResourceAndDefinitionTests.ForegroundTrajectory.cs` locks every family and update subdivision. The runtime's center-target bridge and crossbow lifetime remain **Provisional**. The former universal 70 ms melee frame timer is **Disproved**.
