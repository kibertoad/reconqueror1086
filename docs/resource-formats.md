...
Door-like records occur in adjacent three-record runs with matching kind/name: a closed record (commonly behavior 18 or 1), an intermediate record (commonly behavior 3 or 2), and an open record (behavior 17). The wooden-door runs select `TEX029`, `TEX030`, and `TEX031` respectively. Runtime interaction now preserves collision while displaying the latter two state records, then changes the cell to traversable floor. Triplet adjacency, names, behaviors, and texture sequence are **Confirmed** across the sampled melee/defense scenes; opening duration and the distinction between behavior 1, 18, and 19 remain **Provisional**.
...
The structure, sizes, column ordering, fixed-point viewer position, named block roles, shape texture slots, and kind-4 base-surface slot are **Confirmed for the hashed release** by cross-checking all 12,784 combat-scene block records and locating Viewer positions and coherent images in multiple scene variants. `SKIRMISH.PAL` and the contacted cardinal surface slot now drive runtime wall columns, while kind-4 actors select an angular surface from their active state's base. The palette association is confirmed by executable generation and population-wide byte equality. The intersection routine at `0x44CC0`-`0x44F61` emits masks `0x100`, `0x200`, `0x400`, and `0x800` for the north, east, south, and west faces; the render selector at `0x46739`-`0x467F7` maps them to block offsets 44, 48, 52, and 56 respectively. Palette association and cardinal slot ordering are therefore **Confirmed**. Record offset `0x40` is the state-target block index for placed actionable kind-4 combat records: the update routine at `0x4C900` writes its low word into the contacted map cell. Behavior-bit-`0x20` objects point to floor/ground, while behavior-19 meals point to broken-barrel billboards. The action dispatcher at `0x55524` routes behavior-19's `0x10` bit through a strict `< 0x280` range check at `0x556AF`, a callback at `0x556DA`, and immediate state replacement; with `0x100`-unit cells this is represented as explicit interaction over at most two cells. The dispatch and range are **Confirmed**. The behavior-19 callback table target `0x52D9A` only submits sound ID `0x3FFEA`; food/loot effects are separate and remain **Provisional**. For behavior-135 actors, the signed words at offsets `0x4A` and `0x4C` select one of ten executable-initialized 68-byte combatant templates and one of 25 combat rows; those fields are bound into imported siege spawns. Adjacent door-state and actor-state template structure is described above and below, but adjacent non-door names do not themselves define transitions. The meanings of the remaining block fields, exact heading orientation, and collision variants remain **Provisional**.
