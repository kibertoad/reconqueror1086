...
Combat loader `0x51560` reserves actor records 0-9 as templates, clones placed actors into later 68-byte records, and records the active player actor index in `D4D0`. Routine `0x4D870` then counts records after the templates whose side field `+0x30` is zero and health `+0x40` is positive, excluding `D4D0`; its result is therefore the living-retainer count, not a player-inclusive friendly count. Loader pruning at `0x51B87` compares that count with the cap in `D49C` and only removes actors. It does not generate actors to reach the cap, so the authored-friendly classification remains a distinct open mapping.
