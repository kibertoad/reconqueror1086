...
Actor state transitions are more specific than the current presentation model. `0x4E39C` copies the actor's base or adjacent `+1`, `+2`, or `+3` state block into its live map block; attack and hit paths at `0x502E3` and `0x50436`, and the death path at `0x4EC59`, then attach the copied block's selected SFX descriptor. On completion, `0x53D3F` clears the actor's live-effect handle and calls the actor state routine at `0x4F49C`. In owned `MELEE0.RES`, all of those actor blocks select descriptor 0, whose decoded values are two ticks, 192 ms, flags 5, a `+1` block-index step, and no coordinate, surface, or heading step. It is therefore a nominal 384 ms state-completion gate, doubled to 768 ms for combat rows 23-24; it is not a nine-frame attack cadence. The renderer at `0x46E3B`-`0x46F23` selects a viewing-angle image from the copied billboard block's base surface. Exact use of additional adjacent artwork and the original render rate remain open. Current clean-room sequential attack/collapse playback and door/enemy timing constants remain **Provisional**, and treating either the extra texture count or one universal timer as a recovered cadence is **Disproved**.
...
Placed knight, champion, and footman billboard blocks point to 15-frame texture runs. Local contact-sheet inspection establishes three consecutive poses, each containing five rear-to-front viewing angles; no second-side frames are stored. The adjacent 9-, 3-, and 8-frame runs visibly contain a complete sword strike, rear/side/front impact reactions, and a collapse respectively. Runtime enemies retain cardinal facing, select the angle relative to the viewer, mirror it for the opposite side, advance the three-pose walk cycle when they move, play the sword run when attacking, show the matching impact angle when hit, and remain visible through the collapse after leaving collision/combat. Run boundaries, dimensions, imagery, pose/angle ordering, and the absence of opposite-side frames are **Confirmed**; mirroring direction, initial facing, event association, and cadence are **Corroborated**.
