...
Loader `0x5185F`-`0x51875` writes actor-record coordinates `(cellX << 8) + 0x80` and `(cellY << 8) + 0x80`. Handler `0x4FF53` quantizes heading and passes the descriptor to constructor `0x4C7F4` with flags rewritten as `(flags & 0xA7) | 0x10`. Scheduler `0x530D0` uses strict `elapsed > interval`, rotates local `(64,0)` by actor heading through `0x44740`, and adds it to cloned-block `+0x24/+0x28`. For either axis, with signed delta `d` and retained offset `o`, the core update is `n = o + d`; outside the strict central band `-0x59..0x59`, `0x53425`-`0x535E5` reads the neighboring map block and rejects it when block behavior bit `0x02` is set. Only when accepted `n < -0x80` or `n > 0x80` does it change the map cell and wrap `n` by `+0x100` or `-0x100`; equality at `+/-0x80` stays in the current cell. With official flags `0x142`, rejection reaches `0x53BCE`-`0x53C6C`, zeros the active-axis remainder, and for kind 4 assigns `(((heading + 0x20) & 0xC0) - 0x40) & 0xFF`. Because movement headings are cardinal, this is exactly a 90-degree left turn, and the next scheduler tick rerotates the descriptor delta through it. The actor thinker is recalled only after all three ticks, so arrival and direct re-aim are effect-boundary decisions rather than per-tick decisions.
...
`SiegeActorMovement` retains the tick interval, coordinate delta, flags, and surface stride. `SiegeLayout` carries a separate movement-block plane derived from authored behavior bit 2 rather than semantic visual labels; imported actor starts use their state-target block beneath them, and every object stage carries its own bit so immediate state replacement updates collision at the same time. `SiegeSession.AdvanceRetainerMovement` applies the recovered boundary re-aim, cardinal rotations, strict per-tick deadlines with bounded catch-up, blocker-band rejection, left turn, effect-boundary completion, signed 8.8 remainder, and three walk-surface groups. `SiegeViewProjection.ProjectActors` adds those offsets divided by 256 so rendering and pointer picking share the moving point. Focused tests cover strict equality, first and sustained transition formulas, four cardinal rotations, clockwise diagonal ties, authored blocker masks, left-turn collision, effect-boundary completion, update-frequency independence, imported offsets/profile fields, and sub-cell projection. Corner interaction branches, multi-actor destination behavior, and movement outside selected mode-12 travel remain **Provisional**.
