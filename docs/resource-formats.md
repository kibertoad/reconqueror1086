...
The relocated current-mode handler table at runtime VA `0x4F458` maps modes 1-17 to `4FDAB,4FDAB,4FDAB,4FDAB,4FDCD,4FDCD,4FE76,4FE76,50020,50020,5010B,4FF53,5051A,503EC,504C0,4FE76,4FDCD`. Its preceding acquisition table at runtime VA `0x4F414` maps them to `4F648,4F6F7,4FC34,4F98D,4FC34,4F98D,4F8B0,4F7A2,4FC34,4F98D,4F89B,4FD7B,4FD9A,4F89B,4F89B,4F8B0,4FB39`. Together these tables and their branches confirm the public intents: Defend mode 2 scans only `x-1..x+1` by `y-1..y+1` for an opposite-side actor and attacks without seeking; Attack mode 6 scans actors and replaces its target only on a strictly smaller distance before moving toward it; Follow mode 16 uses the player target installed at `+0x24` and moves toward it; Retreat mode 10 derives motion from `current - target` and moves away. The intent and target-selection direction are **Confirmed**; exact fixed-point cell traversal, obstruction tests, formation behavior, effect scheduling, and call-frequency equivalence remain **Provisional**. `SiegeSession.AdvanceRetainerOrders` maps each confirmed intent to explicit authored actors, uses the already confirmed hit/damage formulas for reachable attacks, and replaces the former global ally-interception probability with spatial nearest-opponent targeting. Its cardinal grid stepping is deliberately provisional and regression-tested in `ResourceAndDefinitionTests.Retainers.cs`.
...
`SiegeActorMovement` retains the tick interval, coordinate delta, descriptor flags, and surface stride. `SiegeLayout` carries a separate movement-block plane derived from authored behavior bit 2 rather than semantic visual labels; imported actor starts use their state-target block beneath them, and every object stage carries its own bit so immediate state replacement updates collision at the same time. `SiegeSession.AdvanceRetainerMovement` applies the mode-12 flag rewrite, recovered boundary re-aim, cardinal rotations, strict per-tick deadlines with bounded catch-up, blocker-band rejection, retained-offset early effect completion/retry, open-path completion boundary, signed 8.8 remainder, and three walk-surface groups. `SiegeViewProjection.ProjectActors` adds those offsets divided by 256 so rendering and pointer picking share the moving point. Focused tests cover strict equality, first and sustained transition formulas, four cardinal rotations, clockwise diagonal ties, authored blocker masks, blocked-effect retry after a state change, effect-boundary completion, update-frequency independence, imported offsets/profile fields, and sub-cell projection. The `0x40` corner/turn branches, multi-actor destination behavior, and movement outside selected mode-12 travel remain **Provisional**.
