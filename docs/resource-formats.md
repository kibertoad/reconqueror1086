...
Placed knight, champion, and footman billboard blocks point to 15-frame texture runs. Local contact-sheet inspection establishes three consecutive poses, each containing five rear-to-front viewing angles; no second-side frames are stored. Runtime enemies retain cardinal facing, select the angle relative to the viewer, mirror it for the opposite side, and advance the three-pose walk cycle when they move. Run boundaries, dimensions, pose/angle ordering, and the absence of opposite-side frames are **Confirmed**; mirroring direction, initial facing, and movement cadence are **Corroborated**. The adjacent 9-, 3-, and 8-frame actor runs remain candidate attack, reaction, and death states pending executable or controlled-observation confirmation.
