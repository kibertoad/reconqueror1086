...
The reimplementation mapping is split deliberately. `SiegeViewProjection.ActorLayout/ObjectLayout` supplies the same geometry used for drawing and picking; `ConquerorGame.SiegePointerWorldHit` resolves nearest visible billboards through current wall depth, draw order, and decoded texture alpha. It consumes the already centered 167x117 source pointer, correcting the former top-left-based world lookup. `SiegeSession.ToggleRetainerSelection`, `CommandSelectedRetainersAt`, targeted `Attack/Shoot`, and targeted `Interact` preserve the returned actor/object identity; object interaction uses the exact `< 0x280` limit and target-specific attacks use recovered contact distances. These dispatch semantics are **Confirmed**, while the current renderer-consistent floating-point projection is **Corroborated** against the active draw path rather than claimed identical to every calculation in `0x470A8`. Exact fixed-point contact coordinates and full original ray traversal remain **Provisional**. `ResourceAndDefinitionTests.PointerTargeting.cs` covers shared draw/pick geometry, off-center actor attacks, exact hostile retargeting, and object action reach.
