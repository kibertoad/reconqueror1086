...
War Planning action `0x36720` requires temporary wealth `1A090 >= 0x50`, subtracts 80 through `0x37340`, and increments the pending assignment count at `1A0EC`. Commit `0x36AFC` tries each pending assignment through `0x38C68`; the first sets the single live-spy flag `AE68`, while another live assignment is rejected. Strategic update `0x3C290` calls report routine `0x38C84`, which scans five `0x118`-byte movement records at object-2 `+0x1AB48`, retains the live spy while none is active, and consumes it when an active record is considered, displaying `Spy Report` when that record resolves. `Campaign.AssignSpy` and `ResolveSpyReports` now preserve the confirmed cost, single-live limit, and one-report lifetime. The monthly report checkpoint remains explicitly Provisional until autonomous enemy-army movement supplies the original trigger. Continue with route recovery, that movement integration, estate tiles, and event audio.
