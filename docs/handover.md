...
Continue priority 1 in `implementation-plan.md`: recover exact first-person foreground presentation before broadening combat behavior. Campaign scene dispatch and door state replacement are now closed.
...
1. Finish exact foreground presentation. The actor scheduler path is implemented from Confirmed evidence: constructor `0x4C7F4` creates up to 64 scheduler records, `0x530D0` advances independent millisecond deadlines, attack/hit/death copy their state block, and renderer `0x46E3B`-`0x46F23` applies the angular-division and mirror formula. All 144 placed actor bases in 42 owned combat scenes use the mapped `8/4/4/2` state shape and imported 384 or 400 ms gates, with ranged attack rows 23-24 doubled. Foreground setup reads combat-row column 3 and calculates signed 8.8 velocity as `(screen displacement << 9) / divisor`; updates integrate elapsed milliseconds. Next recover its exact randomized screen-space start/end geometry and convert the four render-call-counted blood steps into deterministic presentation steps.
