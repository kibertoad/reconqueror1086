...
The last completed implementation gate passed 131 xUnit cases and 145 executable specifications with zero build warnings. Repository policy also passed. Re-run the gate after checkout; do not infer current health from this note alone.
...
1. Finish exact attack cadence and state-linked foreground timing. The architecture and interval source are Confirmed: constructor `0x4C7F4` creates up to 64 scheduler records (`0x40` bytes each), storing current frame at `+0x08`, active/count at `+0x0C`, deadline at `+0x10`, and the selected `SFXDEFS` descriptor's millisecond interval at `+0x14`; `0x530D0` advances one frame after a strict elapsed-greater-than-interval test and preserves overrun by moving the deadline one interval. Scene loading at `0x50AAC`-`0x50ACC` uses the Scenario offset-28 count and loads exactly `count * 0x40` bytes under the literal `SFXDEFS`; owned `MELEE0.RES` contains 11 distinct descriptors with intervals from 100 to 250 ms. The siege loop calls the scheduler at `0x583AA`, then the separate untimed actor thinker `0x50524`, then world rendering. Foreground poses are selected from live state/contact at `0x5529B`-`0x55448`, and blood advances exactly four foreground-render calls at `0x55383`-`0x55395`. Decode `SFXDEFS`, recover its block/state selectors, and replace the disproved universal timing model without translating render-call-counted effects literally.
