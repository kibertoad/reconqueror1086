...
3. Continue authored friendly-placement selection, retainer commands/movement, and broader AI recovery from evidence. Campaign caller `0x39EAA`-`0x39F79` builds one contribution per strategic unit type as `min(floor(units / 3), 3)`, sums them, and supplies a one-retainer fallback; post-return path `0x3A101`-`0x3A1CE` converts missing retainers into one-for-one strategic losses in Swordsmen, Halberdiers, Knights order. `OriginalRetainerCombat`, `SiegeSession`, `Campaign.FinishSiege`, and focused tests implement that Confirmed mapping. The separate `0x399F6` `/50` path reads a non-army actor-domain value through `0x2D500` and must not be relabeled as the campaign army formula. Loader pruning still establishes that `D49C` is a cap over authored friendly actors rather than a generation target. Defeated-enemy loot is closed as Disproved for the supported hashed release: `0x4EA40`-`0x4ECD8` and `0x4F49C`-`0x4F5CE` contain no reward mutation, and full references to wealth `0xD4A4`, ammunition `0xD4A8`, and equipment `0xD4C4` place their combat mutations outside actor death.
