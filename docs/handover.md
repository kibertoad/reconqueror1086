...
Continue priority 1 in `implementation-plan.md`: recover exact first-person foreground/door presentation before broadening combat behavior. The remaining campaign scene dispatch is now closed.
...
1. Finish exact foreground and remaining scene cadence. The actor scheduler path is now implemented from Confirmed evidence: constructor `0x4C7F4` creates up to 64 scheduler records (`0x40` bytes each), `0x530D0` advances independent millisecond deadlines with a strict elapsed-greater-than-interval test, and attack/hit/death paths copy the `+1/+2/+3` state block before attaching its selected descriptor. Renderer `0x46E3B`-`0x46F23` maps relative heading through the state block's angular divisions and behavior-bit-2 mirror rule rather than consuming adjacent textures as frames. A census of all 144 placed actor bases in 42 owned combat scenes finds the same base/attack/hit/death angular shape (`8/4/4/2`) and descriptor 0 on every state: 80 bases use `2 * 192 ms`, 64 use `2 * 200 ms`, and only attack rows 23-24 double the interval. `DynamixSceneEffectDecoder`, `ImportedSiegeLayouts`, `SiegeSession`, the scene renderer, field/formula fixtures, and the full owned-disc pass now preserve those gates and direction rules. Foreground setup reads combat-row column 3 and calculates signed 8.8 velocity as `(screen displacement << 9) / divisor`; updates integrate elapsed milliseconds, so motion is processor-speed independent. Next recover the exact randomized screen-space trajectory and door timing without translating the four render-call-counted blood steps literally.
2. Recover behavior-19 pickup reward semantics separately from its handler callback. The behavior-19 handler at `0x52D9A` is now Confirmed as sound-only (`0x3FFEA`). Explicit-action dispatch, strict `< 0x280` range, callback-before-replacement order, and immediate offset-`0x40` state transition remain Confirmed at `0x55524`-`0x55744` and implemented by `SiegeSession.Interact`/`CollectTile(x,y)` with two-cell-range and no-movement-collection tests. Initialization `0x21DFC`, callback storage at engine field `+0xB0`, its sole invocation at `0x59BC7`, and dispatcher slot 128's literal load at `0x22226` close the campaign scene path as common `MELEE0.RES`.
