...
Continue priority 1 in `implementation-plan.md`: replace the bounded cell-visibility bridge with the recovered non-cardinal fixed-point acquisition ray, then address mode-5 formation transitions and broader AI. Fixed horizontal pointer projection, the 64-step wrapped traversal, exact corner/perpendicular probe order, pass-through and state-target continuation, center/diagonal shapes, decoded block elevation spans, insertion-ordered alpha testing, kind-4 center/depth/heading/mirror projection, and selected-primary dispatch are active. Kind 0 is not a pointer candidate, so the former empty-floor projection is Disproved; mode 12 receives a projected block contact and may target a blocked cell whose entry is governed by movement collision. The GameFAQs FAQ has no comparable internal formula. Movement retains the documented strict 200 ms ticks, signed 8.8 offsets, collision families, and processor-independent update clock.

The projected candidate helper and shared actor-movement paths are the active traces. Switch table `0x34A0C`, the 32-slot but 31-usable `0x44F7C` contact arrays, block geometry fields `+0x10..+0x20`, traversal probe order, behavior-bit-0 continuation, state-target chaining, projection formulas, and primary dispatch precedence are closed. `OriginalSiegeProjection` implements these paths with focused pass-through, shape, state, cap, corner-order, wrap, kind-4 depth, sector, and mirror tests. Pointer picking consumes kind-4 candidates in original order with attached live actor/object identity and alpha. Non-cardinal acquisition still uses the bounded visibility bridge. The movement slice at `0x5185F`-`0x51875` and `0x530D0`-`0x53D5F` includes direct boundary aiming, bit-2 collision, retained partial offset, early effect termination, effect-boundary arrival, and shared-destination occupancy. Earlier confirmed actor-versus-actor portions include:
