...
The current strategic-movement mapping gate passes 309 xUnit cases and 146 executable specifications with zero build warnings. Repository policy passes; re-run the gate after checkout rather than inferring current health from this note alone.
...
War Planning action `0x36720` requires temporary wealth `1A090 >= 0x50`, subtracts 80 through `0x37340`, and increments the pending assignment count at `1A0EC`. Commit `0x36AFC` tries each pending assignment through `0x38C68`; the first sets the single live-spy flag `AE68`, while another live assignment is rejected. Strategic update `0x3C290` calls report routine `0x38C84` immediately before movement update `0x3C088`. The report scans five `0x118`-byte records at object-2 `+0x1AB48` in slot order, retains the live spy while none is active, and consumes it when the first active record is considered. `StrategicEnemyMovement` and `StrategicSpyReport` now persist this five-slot, report-before-advance path; monthly settlement no longer manufactures unknown-garrison intelligence. Follow-up tracing maps the record, property/person tables, named rows, strict generation gate, ordinary force formula, and mode-2 waypoint state. Route builder `0x4A070` selects `rt_<low>_<high>.rat` and reverses its pairs for high-to-low travel; only self-routes and Cambridge/Dunster are absent. Each selected file is an exact little-endian count followed by signed 32-bit x/y pairs. `StrategicRouteDecoder` now bounds and decodes that layout, `OriginalStrategicMovement.PropertyRouteResources` exposes the 90 canonical resource identities, and supported startup requires them all. Generator helper `0x3BDC0` compacts properties with nonzero `+0x00` and `+0x0D == 1` in authored order, returns `-1` when empty, or selects a uniform candidate; `GenerationPropertyCandidates` / `SelectGenerationProperty` preserve that filter without assigning an unproved semantic name to either byte. Completion probes five cells and resolves encounter, matched-origin transfer, or retarget. Retargeter `0x3A9BC` selects the first active field army below 300 units, else the nearer player/origin point with a player-favoring tie. Focused tests preserve these decisions and malformed-input rejection. Generic destination reinforcement is Disproved. The runtime's incompatible indices and dated model remain Provisional; next recover the surrounding generator branches and terrain cadence, make the decoded points drive movement, and plan an explicit save migration before replacing that adapter, then continue estate tiles and event audio.
