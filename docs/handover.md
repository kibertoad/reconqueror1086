...
Do not wire named campaign locations as live player records. The six records are now persisted and executable: `0x11554` drives tagged-target or 21-pair inline-route movement, and outer `0x13168` moves each record before strict `<30` enemy contact, selection, avatar cooldown alerts, and the distinguished army's five-cell trigger. `AE64` is the selected record across all six; `AE6C` is the distinct distinguished army index. This corrects the prior label. The next cohesive recovery batch should map the input/constructor paths that populate routes and non-enemy target handles, then bind division forces/avatar travel and application presentation before activating schema 2. Do not push this continuation batch to `main`.
