...
Continue priority 1 in `implementation-plan.md`: implement the original multi-candidate order, diagonal/center-plane shapes, wrapped cropped-map lookup, and corner selection, then later mode-5 formation transitions and broader AI. Fixed horizontal projection, 64-step traversal, decoded block elevation spans, alpha testing, and selected-primary dispatch are active. Kind 0 is not a pointer candidate, so the former empty-floor projection is Disproved; mode 12 receives a projected block contact and may target a blocked cell whose entry is governed by movement collision. The GameFAQs FAQ has no comparable internal formula. Movement retains the documented strict 200 ms ticks, signed 8.8 offsets, collision families, and processor-independent update clock.

The projected candidate helper and shared actor-movement paths are the active traces. Switch table `0x34A0C`, the 32-entry `0x44F7C` contact arrays, block geometry fields `+0x10..+0x20`, projection formulas, and primary dispatch precedence are closed. Runtime first-solid contact remains Corroborated; candidate continuation after transparent pixels, cropped-layout wrapping, diagonal surfaces, and simultaneous-corner order remain open. The movement slice at `0x5185F`-`0x51875` and `0x530D0`-`0x53D5F` includes direct boundary aiming, bit-2 collision, retained partial offset, early effect termination, effect-boundary arrival, and shared-destination occupancy. Earlier confirmed actor-versus-actor portions include:
