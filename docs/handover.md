...
The schema-2 person-population prerequisite is closed. Inspector output now enumerates every field of all 176 executable-initialized 18-byte person rows; `OriginalStrategicMovement.Persons` retains those rows as immutable typed definitions. The compact census hash is `ec3a31ce97eb367cf3a39e033e2d9b6dccdfd72769d590a0ce9a839169754b4f`, and tests independently bind the 14 property lords, seven starting castles, and per-group eligible household totals back to it. Save/load `0x42E4C`-`0x43000` resets the byte-sized person list links and transfers the mapped dynamic slices, while property links remain 16-bit. Next build mutable schema-2 copies of these rows with the five exact movement slots, camera/terrain state, and deterministic schema-1 settlement; do not alias the static definitions or advance the schema version before round-trip validation passes.
