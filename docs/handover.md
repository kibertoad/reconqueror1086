...
Continue priority 1 in `implementation-plan.md`: address mode-5 formation transitions and broader AI. Fixed-point pointer projection and non-cardinal actor acquisition are active with wrapped traversal, exact candidate order, decoded elevation spans, texture alpha, live actor identity, and authored-order nearest selection. Kind 0 is not a pointer candidate, so the former empty-floor projection is Disproved; mode 12 receives a projected block contact and may target a blocked cell whose entry is governed by movement collision. The GameFAQs FAQ has no comparable internal formula. Movement retains the documented strict 200 ms ticks, signed 8.8 offsets, collision families, and processor-independent update clock.
