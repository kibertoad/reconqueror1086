...
The latest completed implementation gate passed 182 xUnit cases and 146 executable specifications with zero build warnings. Repository policy also passed. Re-run the gate after checkout; do not infer current health from this note alone.
...
Continue priority 1 in `implementation-plan.md`: recover empty-ground mode-12 orders, exact fixed-point ray/actor traversal and pathfinding, and broader AI rules. Visible actor/object click identity, authored friendly placement, the campaign cap and strategic losses, all four command identities, dispatch rules, and local-defend/seek/follow/flee intents, campaign scene dispatch, doors, pickups, absence of enemy drops, foreground coordinates/trajectories, and processor-independent blood presentation are closed at the currently available fidelity.
...
The older item 3 wording below is superseded for command dispatch, intent, and visible actor/object clicks. The shell band `x=4..209`, `y=175..187` and handlers `0x5700E/0x57168/0x572C5/0x5744B` confirm Attack 6, Defend 2, Follow 16 targeting player `D4D0`, and Retreat 10, with selected-or-all dispatch and selection clearing. Thinker/state paths `0x50524/0x4F49C` and runtime dispatch tables `0x4F414/0x4F458` confirm Defend's 3x3 reaction, Attack's nearest-hostile search, Follow's approach, and Retreat's away vector. World dispatcher/raycaster `0x55524/0x470A8` confirms exact actor/object identity, pixel-mask testing, friendly selection, mode-8 hostile targeting, and the `< 0x280` object action gate. Runtime authored retainers, pointer selection, targeted actions, command UI, perspective/radar presentation, cap formula, and army-loss conversion are active. Continue with empty-ground mode 12, formations, exact fixed-point ray/obstruction/path selection, cadence equivalence, and broader AI.
