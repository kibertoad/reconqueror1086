...
3. Continue retainer commands/movement and broader AI recovery from evidence. Authored side selection is now closed for the supported official scene population: loader `0x51560` uses behavior bit `0x80` plus selector 1, scans x-major, and promotes the first friendly to player `D4D0`; the complete 1,002-placement census partitions templates 0-2 as friendly and 3/5/8/9 as hostile. `ImportedSiegeLayouts` preserves those authored positions and `SiegeSession` caps them with the campaign formula. Campaign caller `0x39EAA`-`0x39F79` builds one contribution per strategic unit type as `min(floor(units / 3), 3)`, while `0x3A101`-`0x3A1CE` converts missing retainers into one-for-one strategic losses in Swordsmen, Halberdiers, Knights order. The separate `0x399F6` `/50` path remains actor-domain, not the campaign formula. Raw surface/color normalization is still unmapped. Defeated-enemy loot remains Disproved for the supported hashed release.
