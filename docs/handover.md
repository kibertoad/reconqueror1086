...
Continue priority 1 in `implementation-plan.md`: finish processor-independent blood presentation, then recover pickup rewards before broadening combat behavior. Campaign scene dispatch, door state replacement, and the melee foreground trajectory are now closed at the currently available input-binding fidelity.
...
1. Finish exact foreground presentation. `0x54C07`-`0x54F12` now supplies the active row-family random spans, viewport-relative target/outer coordinates, and mirror decisions; `0x54F98`-`0x5501E` supplies the active signed 8.8 velocity and elapsed-millisecond integration; `0x550F2`-`0x55140` supplies contact reversal; and `0x5529B`-`0x55448` supplies the active position/contact pose rule. `SiegeForegroundTrajectory` retains fixed-point subpixels and uses the original 166 ms update cap, with family and frame-subdivision coverage in `ResourceAndDefinitionTests.ForegroundTrajectory.cs`. The current keyboard combat bridge aims at the nearest projected target center; exact original input-coordinate provenance and crossbow lifetime remain Provisional. Next replace the four render-call-counted blood steps with deterministic presentation steps after recovering their intended cadence.
