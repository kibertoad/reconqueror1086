...
The active implementation branch is `reimplementation-full-migration`. Before the current sub-cell batch its verified remote tip was `2b9739d` (`Recover original actor movement cadence`); use Git and the full gate below as authority rather than assuming this note names the newest commit.
...
The latest completed implementation gate passed 190 xUnit cases and 146 executable specifications with zero build warnings. Repository policy also passed. Re-run the gate after checkout; do not infer current health from this note alone.
...
Continue priority 1 in `implementation-plan.md`: recover exact fixed-point map-ray ground selection, route/collision behavior, actor-wide movement cadence, multi-retainer destination routing, and broader AI rules. Selected mode-12 ground travel now preserves imported strict 200 ms ticks, signed 8.8 offsets, five-surface walk stride, strict collision/cell bands, retained remainder, and shared draw/pick position. The first zero-offset transition takes the three-tick 600 ms effect cycle; sustained cells take four ticks/800 ms, disproving the earlier universal 600 ms deadline. Visible actor/object click identity, authored friendly placement, campaign cap/losses, four command identities and intents, campaign scene dispatch, doors, pickups, absence of enemy drops, foreground trajectories, and processor-independent blood presentation are closed at their documented fidelity.

The fixed-point ray/candidate and shared actor-movement paths are the active traces. The movement slice at `0x5185F`-`0x51875` and `0x530D0`-`0x53711` is now mapped into selected ground travel; exact route decisions and other modes remain open. Earlier confirmed actor-versus-actor portions include:
...
The older item 3 wording below is superseded for command dispatch, intent, visible actor/object clicks, and selected-ground sub-cell presentation. The shell band `x=4..209`, `y=175..187` and handlers `0x5700E/0x57168/0x572C5/0x5744B` confirm Attack 6, Defend 2, Follow 16 targeting player `D4D0`, and Retreat 10, with selected-or-all dispatch and selection clearing. Thinker/state paths `0x50524/0x4F49C` and runtime dispatch tables `0x4F414/0x4F458` confirm Defend's 3x3 reaction, Attack's nearest-hostile search, Follow's approach, and Retreat's away vector. World dispatcher/raycaster `0x55524/0x470A8` confirms exact actor/object identity, pixel-mask testing, friendly selection, mode-8 hostile targeting, and the `< 0x280` object action gate. `0x470A8` itself is now confirmed as a candidate projection/pixel resolver fed by the 64-step traversal at `0x45158`, not a standalone half-cell floor formula. Runtime authored retainers, pointer selection, targeted actions, command UI, perspective/radar presentation, cap formula, army-loss conversion, and mode-12 sub-cell motion are active. Continue with formations, exact fixed-point empty-ground return coordinates, obstruction/path selection, multi-actor routing, cadence in all actor modes, and broader AI.
