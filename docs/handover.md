...
War Planning action `0x36720` requires temporary wealth `1A090 >= 0x50`, subtracts 80 through `0x37340`, and increments the pending assignment count at `1A0EC`. Commit `0x36AFC` tries each pending assignment through `0x38C68`; the first sets the single live-spy flag `AE68`, while another live assignment is rejected. Strategic update `0x3C290` calls report routine `0x38C84` immediately before movement update `0x3C088`. The report scans five `0x118`-byte records at object-2 `+0x1AB48` in slot order, retains the live spy while none is active, and consumes it when the first active record is considered. `StrategicEnemyMovement` and `StrategicSpyReport` now persist this five-slot, report-before-advance path; monthly settlement no longer manufactures unknown-garrison intelligence. Follow-up tracing maps the record through `+0x68`, proves a strict post-5,000-ms `random(100) > 96` generation gate, and recovers `0x38A8C`'s ordinary equal-force formula as `active household + floor(lord byte / 4)`, with a `0/0/1` minimum. The exact 14-row property table at object-2 `+0xB8EC`, 176-record person layout at `+0xBA50`, household eligibility predicate, and initial group counts are also mapped and preserved by `OriginalStrategicMovement`. Linked person-name pointers identify the rows in order as York, Lincoln, Chester, Gloucester, Warwick, Oxford, Cambridge, London, Norwich, Colchester, Arundel, Windsor, Dunster, and Okehampton; their force-input bytes are preserved too. The runtime's incompatible location indices, daily generation, route choice/detachment, and dated arrival remain Provisional; next recover mode-2/arrival transitions and plan an explicit save migration before replacing that adapter, then continue estate tiles and event audio.
