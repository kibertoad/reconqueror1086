...
The importer and runtime catalog are format-neutral: assets are addressed by original path and kind, and the game rejects manifest paths that escape `UserContent`. The current runtime plays imported CD audio, renders validated PCX-compatible title, map, and portrait art, and can decode validated CSF sequences and palettes without a graphics device. SMK playback, kind-2 payloads, and the remaining dialogue/image formats are still adapter work; the presence of an imported asset must not be reported as equivalent to displaying or playing it. On the hashed GOG build, an end-to-end import after kind-1 support produced 16,092 catalog entries. The installer decodes all byte-stored and kind-1 entries from the GOB and 50 scene containers; the five unequal-size kind-2 GOB entries remain only in their owning archive.
...
7. Parses the bounded top-level Dynamix archive directory and reports names, flags, sizes, and offsets. For kind-1 entries it validates and decodes every two-byte-length-prefixed block and its expected 16 KiB output slice. The end-user importer writes byte-stored and fully decoded kind-1 entries; kind 2 remains in its owning archive.

`gob-compression-report.txt` is a generated local report. On the hashed build, all 471 kind-1 entries divide exactly into 2,963 expected 16 KiB output slices: 2,940 compressed-marker blocks and 23 verbatim blocks. The scene report adds 17,410 compressed-marker and eight verbatim blocks. All 20,381 blocks decode to their exact expected sizes with the bounded LZ/RLE decoder. Five unequal-size kind-2 entries remain separately identified. The reports contain names and structural results, not extracted payloads. The shared resource library also contains a bounded Dynamix LZW decoder for documented inner chunk streams; real-data probes disproved both raw LZW and classic LH1 as interpretations of the outer kind-1 data.

`scene-res-report.txt` repeats the same bounded inventory directly from all 50 scene containers in the ISO without writing their payloads. `resource-extension-report.txt` aggregates storage kinds, and `stored-image-report.txt` validates stored and kind-1 PCX-compatible images using dimensions and pixel-index hashes. It identifies 188 strict PCX payloads, including `richard.pcc`, `fftitle.pcx`, and `engmap1.pcx`. The runtime catalog decodes installed PCX-compatible assets into bounded indexed pixels and RGBA data without creating a graphics device.
...
Runtime art selection is definition-driven. `ImportedArt.Definitions` maps stable presentation roles to an imported kind and original identifier suffix; loading and disposal are generic. Active roles include the title, character-options, pre-generated-character, load-game, England-map, and tournament-portrait screens, so extending presentation does not require asset-name conditionals in the loader.
...
`hat-layout-report.txt` reproducibly decodes every supported HAT descriptor into its screen identifier, origin, dimensions, background name, unknown three-byte tag, and ordered region records. The runtime reads the installed `CGOPTS.HAT` and `PREGEN.HAT` through the same bounded parser, replacing fallback rectangles by numeric region identifier without embedding proprietary bytes.
