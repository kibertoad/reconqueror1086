...
`smacker-report.txt` validates the fixed header, frame tables, tree extent, exact aligned frame extents, palette updates, audio packets, and remaining video packet of every `.SMK` without exporting movie content. All 2,131 files are `SMK2`: 2,095 have one packed 8-bit mono track and 36 are silent. Together they contain 182,360 indexed frames, 2,135 palette changes, and 161,884 audio packets in 288,867,980 source bytes. The bounded predictive-Huffman decoder expands every audio packet to its declared length, totaling 398,368,812 unsigned PCM bytes. This homogeneous population supports direct decoding without an installation-time transcoding dependency.
