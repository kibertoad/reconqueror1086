...
The importer and runtime catalog are format-neutral: assets are addressed by original path and kind, and the game rejects manifest paths that escape `UserContent`. The current runtime plays imported CD audio, renders validated PCX-compatible title, character, dilemma, map, and portrait art, decodes validated CSF sequences and palettes without a graphics device, and binds all 30 marker-delimited dilemma definitions to the executable-confirmed six-age selection and outcome interpreter. Original prose remains in ignored local content. SMK playback and the remaining dialogue/image formats are still adapter work; the presence of an imported asset must not be reported as equivalent to displaying or playing it. On the hashed GOG build, an end-to-end import after kind-2 support produces 16,097 catalog entries. The installer decodes every byte-stored, kind-1, and kind-2 entry from the GOB and all byte-stored/kind-1 entries from the 50 scene containers.
