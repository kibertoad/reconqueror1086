...
For scene semantics, `--scene-blocks` emits the ignored `scene-block-report.txt` and stops before the expensive movie population pass. The report records bounded block fields and map-placement counts for owned `MELEE*` and `DEFEND*` archives; it is local analysis material and must not be committed.
