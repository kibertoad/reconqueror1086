...
`sound-bank-report.txt` validates all 26 `.666` banks across the GOB and scene archives and records only provenance, sample counts, rates, and aggregate byte sizes. The 102 framed samples consume every decoded bank exactly. `VSMITH.666` is therefore known to be audio rather than the blacksmith dialogue database. `ALL.CIF` and `ALL.CBF` are the actual indexed conversation-node family: all 1,311 nodes, 2,062 prompt variants, and 2,696 response branches validate structurally, while conditions, mutations, and entry-point selection remain open.
