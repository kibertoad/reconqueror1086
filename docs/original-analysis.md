...
`smacker-report.txt` validates the fixed header, frame tables, tree extent, and exact aligned frame extents of every `.SMK` without decoding or exporting movie content. All 2,131 files are `SMK2`: 2,095 have one packed 8-bit mono track and 36 are silent. Together they contain 182,360 indexed frames in 288,867,980 source bytes. This homogeneous population supports pursuing a bounded direct decoder before adding any installation-time transcoding dependency.
