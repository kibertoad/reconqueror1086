...
`smacker-report.txt` validates the fixed header, frame tables, tree extent, exact aligned frame extents, palette updates, audio-packet extents, and remaining video packet of every `.SMK` without exporting movie content. All 2,131 files are `SMK2`: 2,095 have one packed 8-bit mono track and 36 are silent. Together they contain 182,360 indexed frames, 2,135 palette changes, and 161,884 audio packets in 288,867,980 source bytes. This homogeneous population supports pursuing a bounded direct decoder before adding any installation-time transcoding dependency.
