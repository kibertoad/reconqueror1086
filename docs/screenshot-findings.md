...
| Options/start hub | The interactive hub includes New Game, Load, Save, Practice, Resume, Exit DOS, CD music, MIDI music, sound effects, digitized speech, animation, credits, and movie controls. It follows the introductory title rather than being the title bitmap itself. | `GAMEOPTS.HAT` declares screen 1 with `OPTFIN.PCX`; its 13 regions are bound to typed actions. `OPTION.CSF` frames 0/1 and 2/3 are the enabled/disabled released/held setting widgets and frame 4 is Resume; activation now requires release within the pressed region. Exact executable branch confirmation remains outstanding. | Direct visual controls and resources Confirmed; action mapping and input timing Corroborated. |
| Practice menu | Practice is a full-screen outdoor training ground with siege engines, a jousting lane, troops, a castle, and an Exit sign. | `PRACTICE.HAT` declares screen 15 with `PRACTICE.PCX` and five matching regions. A contiguous executable table labels them War, Joust, Melee, Exit, and Castle Skirmish; `JOUSPRAC.SMK` is the joust-practice movie. All branches are active through isolated tactical or first-person sessions, but exact combat assets and parameters remain provisional. | Art, geometry, labels, and movie Confirmed; region ordering and engine families Corroborated. |
...
| Castle office/Home | Home is a full-screen castle office with desk, map, orders, model keep, and exits embedded in the room. | Decoded `TACTICAL.PCX` matches the supplied screenshot exactly. `FOPTS.HAT` is screen 16, names `TACTICAL.PCX`, and defines ten object/exit regions. A contiguous ten-label executable catalog plus the known Overview/Castle, three-ledger, and Map geometry anchors corroborate the complete ordinal region mapping. | Background/descriptor Confirmed; complete ordinal action mapping Corroborated. |
...
- **Implemented:** all ten Home regions follow the executable's ordered label catalog: Overview, Castle, Farm, Village, Forest, War Planning, Exit, JUMP!!, Map, and Orders. Nine have executable-registered actions; `JUMP!!` remains visible but is intentionally label-only. The first five and Map geometries independently anchor the ordinal interpretation; the original `F_OVER.PCX` and `WARPLAN.PCX` backgrounds are active for the corresponding destinations.
- **Implemented:** decode and render all 22 `WARPLAN.CSF` controls through the exact `FWARPLAN.HAT` rectangles. The manual-confirmed five-division roster persists named forces, left/right row clicks add/remove 100-serf companies, the 60-company and away-from-home restrictions are enforced, Join/Leave and Field Army state persist, and OK/Cancel commit or restore the complete plan. Spy assignment now uses the executable-confirmed 80-shilling cost, one-live-spy limit, and one-report lifetime; the monthly report checkpoint remains a compatibility proxy for the original enemy-movement trigger.
...
- Recover exact path interaction and captain battle rules for the now-active independently fielded divisions, then bind the confirmed spy report to autonomous enemy-army movement instead of the monthly compatibility checkpoint. Recover behavior/cost semantics for currently display-only management rows and replace the provisional terrain fill with editable tiles. The former `JUMP!!` effect gap is closed: executable callback registration proves the region has no action.
