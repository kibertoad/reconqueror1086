...
- **Implemented:** route Village to the screenshot-confirmed populated inn; source ten patron actions, Exit, and the label scroll from `VINN.HAT`; and bind the executable-ordered names and matching PCC portraits. Patron selection now opens the original generic conversation frame with the correct portrait. Dialogue prose remains explicitly unavailable until its records are recovered.
