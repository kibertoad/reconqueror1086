...
- Recover exact path interaction and captain battle rules for the now-active independently fielded divisions, plus exact spy timing/cost; then recover behavior/cost semantics for currently display-only management rows and replace the provisional terrain fill with editable tiles. The former `JUMP!!` effect gap is closed: executable callback registration proves the region has no action.
