...
`CurrentSaveSchemaVersion` intentionally remains 1 and `Campaign.AdvanceDays` still uses the isolated dated adapter. The remaining activation gate is application wiring: source five live player-army targets, global person/origin and reactive-finder inputs from campaign state, preserve spy-report-before-scheduler ordering, hand encounters to presentation, drive one pass from the fixed update, and cover load/save integration. Only after that path replaces the adapter may migration be invoked from `Campaign.Load` and the schema advance to 2.
