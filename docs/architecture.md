...
`OriginalStrategicMovement.AdvanceMovementPass` is the mutable motion boundary. It validates replacement state, resolves physical slots 0-4 regardless of serialized list order, dispatches modes 1/2/3 once, consumes sparse terrain mutations over the immutable grid, and reports the original completion signal independently from post-handler active state. Mode 1 retains normalized direction and the executable's 0.9 factor; mode 2 consumes the decoded cursor and stores scaled direction; mode 3 reads one of five supplied live player-army targets and can switch to mode 1 at impassable terrain. This kernel intentionally does not generate records or resolve completion contacts yet, so it cannot be wired to the game update in isolation.

`CurrentSaveSchemaVersion` intentionally remains 1 and `Campaign.AdvanceDays` still uses the isolated dated adapter. The remaining activation gate is the scheduler shell around the motion kernel: report-before-generation ordering, exact constructors, supplied player-army motion targets, completion/contact resolution, encounter handoff, and fixed-update/load-save tests. Only after that path replaces the adapter may migration be invoked from `Campaign.Load` and the schema advance to 2.
