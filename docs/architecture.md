...
Schema-1 saves contain neither a starting-home selector nor strategic camera coordinates. Migration therefore records selector `-1` as an explicit unavailable sentinel and camera `(0,0)` as a deterministic compatibility default; it derives the terrain profile from the saved month and clamps the existing 1-15 UI speed into the exact range. New schema-2 games will instead choose and persist one of the seven valid `StartingRoutes`. The immutable base terrain remains required resource 292 `icon.jp`; sparse mutations replace the process-global `temp.jap` scratch copy. Camera state is material because inclusive shared-edge pixels resolve by current wrapped draw order: 34 owned-route occurrences at six coordinates differ between opposite camera extremes. The replacement state and settlement now validate end to end, but `CurrentSaveSchemaVersion` intentionally remains 1 and `Campaign.AdvanceDays` still uses the isolated dated adapter. The remaining activation gate is a Game-to-Core provider for decoded route/grid data plus the fixed-update five-slot scheduler; only after that path replaces the adapter may migration be invoked from `Campaign.Load` and the schema advance to 2.
