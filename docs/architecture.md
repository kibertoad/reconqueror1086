...
Before enabling exact movement, schema-2 load will settle every active schema-1 column deterministically by returning its complete troop total to its still-hostile schema-1 origin garrison when that origin remains valid, otherwise dispersing it, and recording the outcome in the campaign journal. It will then clear the dated roster and initialize the 14 original properties and all 176 original person rows, five exact movement slots, generator accumulator/counters, current starting-home selector, strategic camera row/column, and calendar-derived terrain profile from typed definitions. `OriginalStrategicMovement.Persons` now closes the complete numeric initialization source, including the byte-sized person list link and neutrally named state bytes; schema state must copy mutable rows rather than aliasing this immutable definition table. New schema-2 games will choose and persist one of the seven `StartingRoutes` during original property initialization. Exact route records will persist resource identity plus waypoint index and retained floating/sub-cell state so save/load cannot restart a path or change its cadence. The immutable base terrain comes from required resource 292 `icon.jp`; mutable cell changes formerly written to `temp.jap` belong in schema 2 rather than a process-global scratch file. Strategic speed is a persisted 1-15 multiplier applied once per runtime fixed update, replacing the original processor-dependent unrestricted-loop cadence while preserving its arithmetic. This one-way policy preserves aggregate player-visible forces where possible without fabricating original state that schema 1 never recorded. Terrain source, clock identity, full person population, and `0x629B0/0x6E5A0` world-to-grid inversion are now closed. Camera state is material because inclusive shared-edge pixels resolve by current wrapped draw order: the complete owned route population contains 34 occurrences (six distinct coordinates) whose selected cell differs between opposite camera extremes. Implementation remains pending on end-to-end schema-2 replacement-state validation, and the schema version must not advance before that passes.
