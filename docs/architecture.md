...
`CurrentSaveSchemaVersion` intentionally remains 1 and `Campaign.AdvanceDays` still uses the isolated dated adapter. The remaining activation gate is the fixed-update five-slot scheduler and its load/save transition tests; only after that path replaces the adapter may migration be invoked from `Campaign.Load` and the schema advance to 2.
