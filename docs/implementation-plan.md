...
2. Continue tracing startup/menu input timing and HAT region-action dispatch from `CONQUER.EXE`; all ten Home labels, nine registered Home actions, War Planning controls, and independent division movement are active. `JUMP!!` is now closed as an intentionally inactive label-only region. Exact path interaction, spy timing/cost, and original input timing remain to be confirmed.
