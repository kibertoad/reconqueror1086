...
Combat recovery update (2026-09-13, empty-ground orders): pointer branch `0x555BE`-`0x5562E`, acquisition `0x4FD7B`, and handler `0x4FF53` confirm that requested mode 12 stores integer target coordinates in actor `+0x28/+0x2C`, affects selected actors only, clears selection and target actor `+0x24`, derives heading from `target - current`, and completes only when both current coordinates match. The runtime now maps below-horizon centered pointer coordinates onto visible floor cells, rejects destinations behind DDA wall depth, sends selected retainers to that exact cell, and resumes their previous command on completion. The mode semantics are **Confirmed**; half-cell floor-plane projection, fixed-point rounding equivalence, cardinal traversal, and multi-actor collision routing are **Provisional** and remain in the exact-ray/pathfinding workstream.

Combat recovery update (2026-09-13, actor movement cadence): mode-12 handler `0x4FFBC` obtains its movement descriptor from actor-block signed word `+0x46` (the high word at `+0x44`), not the visual state selector at `+0x2E`. The complete official combat census finds 134 actor bases selecting descriptor 7 and ten selecting descriptor 10; all 144 bases and 1,002 placements resolve to the identical profile of three ticks, 200 ms, flags `0x142`, and local 8.8 delta `(64,0)`. Scheduler `0x530D0` rotates the local delta by quantized heading, advances only when elapsed is strictly greater than the interval, and changes the map cell on the third 64-unit substep, so `3 * 200 = 600 ms` is the nominal cell deadline. `DynamixSceneBlock.MovementEffectDefinitionIndex`, the scene-block census, `SiegeActorMovement`, imported spawns, and `AdvanceRetainerMovement` carry this **Confirmed** mapping into selected destination orders on a processor- and input-independent clock. Exact map-ray coordinates, intermediate sub-cell presentation, route selection, obstruction deflection, and multi-actor collision remain **Provisional**.
