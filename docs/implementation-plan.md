...
Non-cardinal acquisition integration remains Corroborated. The trusted GameFAQs FAQ 66730
contains no internal ray mathematics and is not corroboration here.

1. Continue first-person combat recovery: projected surface contacts now use fixed horizontal columns, decoded block elevations, texture alpha, selected-primary dispatch, exact multi-candidate order, diagonal/center-plane shapes, wrapped lookup, simultaneous-corner probes, and kind-4 live-center/heading/mirror projection; the former empty-floor return is Disproved. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct cardinal re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic authored-order tie-breaking are active. Next replace the bounded cell-visibility bridge with the non-cardinal fixed-point acquisition ray, then close mode-5 formation transitions and broader enemy AI. Handler `0x50020` remains excluded because the complete supported population cannot enter it.
