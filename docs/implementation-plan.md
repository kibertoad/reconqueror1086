...
- [x] Replace modulo-three campaign scene selection with a stable provisional 15-castle mapping across `MELEE00`-`MELEE04`, `MELEE10`-`MELEE14`, and `MELEE20`-`MELEE24`. Executable tracing places the two-digit builder in tournament melee, so the exact campaign and London mappings remain Provisional.
- [x] Decode and palette-verify the 53-frame `SKIRMISH.CSF` sequence, classify its weapon, shield, and blood-effect runs, and bind the axe, crossbow, hammer, mace, sword, and dagger sequences to first-person combat. Executable dispatch now selects base 43 for fatal target hits and base 48 for nonfatal target hits and renders offsets zero through three. Exact frame cadence and equipment-category edge cases remain Provisional.
...
1. Continue first-person combat recovery: confirm London/base-scene selection, foreground frame cadence, and weapon-category dispatch; then recover non-door block transitions and replace the remaining provisional combat, AI, retainer, loot, and consequence rules.
