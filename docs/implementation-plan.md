...
| M4: Conquest | 6-7 | Playable prototype: the manual-confirmed five-division War Planning roster, company editing, membership, field state, one-live one-report spies, joined-division combat, persisted captain-commanded movement, persisted five-slot autonomous enemy movement with report-before-advance spy binding, and original scene-map/start/spawn ingestion are active. The original movement record layout, 5,000 ms generation gate, strict roll, authored-order eligible-property filter, ordinary lord/household force initializer, exact 14-row named property table, linked lord inputs, person-record layout, initial eligible household counts, canonical waypoint resources, exact bounded route decoder, mode-2 state, five-cell completion probe, and encounter/transfer/retarget precedence are mapped and regression-tested; startup now requires all 90 executable-selected paths from a supported import, while the runtime still isolates its daily dated-travel adapter. Campaign assaults use the confirmed common `MELEE0.RES`, while practice randomly selects the three executable-confirmed unsuffixed base scenes and tournament's separate two-digit family is no longer misused. Imported first-person scenes render inside the original combat shell and viewport with cropped connected maps, panoramas, executable-cardinal and distance-shaded wall faces, explicit door state targets, direction-aware actor state templates with imported completion gates, exactly bound equipped-weapon foreground attacks, original per-weapon break rolls, and executable-dispatched fatal/wounding hit effects with occlusion. | Recover the full generator branch policy and terrain cadence, plan migration from incompatible runtime location indices, bind the exact strategic clock and make decoded routes drive movement instead of the dated adapter; captain battle rules, remaining original strategic rules, status/control rules, exact foreground presentation, combat balance and AI, retainers, and loot. |
