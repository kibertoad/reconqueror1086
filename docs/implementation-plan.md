...
`OriginalStrategicCampaignState` now provides the non-aliased mutable replacement specified by `architecture.md`: all 14 properties, all 176 people, five exact indexed slots, route identity/reversal/cursor, integer and floating movement state, generator/reactive counters, bounded speed, month-derived profile, persisted camera, list heads, and sparse full-dword terrain mutations. `StrategicSchemaTwoMigration.Prepare` implements deterministic slot-ordered schema-1 settlement, returning complete columns only to valid still-hostile origins and journaling all return/dispersal outcomes before clearing the dated roster. Complete JSON round-trip and structural rejection tests pass. The next checkpoint establishes `IOriginalStrategicResources` in Core and an eager Game adapter over all 97 verified route resources plus `icon.jp`; initial, custom, pregenerated, and loaded campaigns share it, and prepared saves must match decoded route lengths. This checkpoint deliberately does not change `CurrentSaveSchemaVersion` or `Campaign.AdvanceDays`; next implement the fixed-update five-slot scheduler, replace the dated adapter, and only then activate migration and schema 2.
