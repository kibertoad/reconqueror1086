...
- [x] Apply the owned manual's War Planning rules: persist five named divisions, edit 100-serf companies with a 60-company cap and home-territory restriction, stage field/join/spy state transactionally, charge all-division upkeep, preserve the confirmed 80-shilling single-live one-report spy lifetime through a provisional monthly report trigger, and make OK/Cancel commit or restore every pending change.
...
2. Continue tracing startup/menu input timing and HAT region-action dispatch from `CONQUER.EXE`; all ten Home labels, nine registered Home actions, War Planning controls, and independent division movement are active. `JUMP!!` is now closed as an intentionally inactive label-only region. Spy cost and lifetime are confirmed; exact path interaction, autonomous enemy-movement report binding, and original input timing remain to be confirmed.
