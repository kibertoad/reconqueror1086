...
Combat recovery update (2026-09-13, retainer command behavior): actor thinker `0x50524` budgets `(actor count - 10) >> 4` records per main-loop pass and feeds state routine `0x4F49C`. The relocated acquisition and handler tables at runtime VAs `0x4F414` and `0x4F458`, together with transition helper `0x4E5F0`, confirm that Defend mode 2 scans only the surrounding 3x3 cells and attacks without seeking, Attack mode 6 chooses a strictly nearer hostile and moves toward it, Follow mode 16 moves toward player target `D4D0`, and Retreat mode 10 derives motion away from its hostile target. These intents are **Confirmed** for the hashed release. `SiegeSession.AdvanceRetainerOrders` now drives the authored friendly records explicitly and hostile actors select spatial opponents, eliminating the former aggregate interception probability. Current cardinal one-cell movement is **Provisional** pending exact recovery of fixed-point traversal, obstruction/path selection, formation behavior, and thinker cadence equivalence. `ResourceAndDefinitionTests.Retainers.cs` covers each intent, Defend's neighborhood, and explicit friendly targeting.
...
Combat recovery update (2026-09-13, mode-12 routing/collision): handler `0x4FF53` passes `target - current` through the piecewise integer heading helper `0x445C4`, then applies `(heading + 0x20) & 0xC0`; the four exact diagonal headings therefore choose the clockwise cardinal. This aim is recomputed only when the three-tick effect returns to actor logic. Scheduler `0x53425`-`0x535E5` rejects an axis after its proposed signed offset leaves `-0x59..0x59` when the neighboring map block has behavior bit `0x02`; with official flags `0x142`, `0x53BCE`-`0x53C6C` zeros that axis remainder and changes a kind-4 heading by `-0x40`, a 90-degree left turn, before the next tick rerotates `(64,0)`. Arrival is likewise observed only after effect completion, even when a cell crossing occurred earlier in the cycle. The complete official scene census finds 52,278 bit-2-blocked and 635,850 clear placements across all 688,128 cells, while all 1,002 actors restore clear underlying state-target blocks. `SiegeLayout` now carries this authored blocker plane independently of semantic visuals, object transitions update it, and mode-12 movement implements the direct aim, strict collision band, left turn, and completion boundary with focused tests. These details are **Confirmed** for the hashed release. Exact ground-ray coordinates, scheduler corner-contact behavior, multiple actors sharing a destination, other actor modes, and broader AI remain **Provisional**. The GameFAQs guide does not specify these low-level routing rules and therefore neither corroborates nor conflicts with this promotion.
...
1. Continue first-person combat recovery: the original four render-counted blood steps are mapped to a processor-independent compatibility timer because executable analysis proves no stable original duration exists. Pointer-to-foreground provenance, direct visible actor/object identity, empty-ground mode-12 semantics, its strict imported 200 ms ticks and retained 8.8 presentation offsets, direct cardinal re-aim, authored bit-2 blockers, left-turn collision rejection, crossbow lifetime, authored friendly-placement selection, campaign retainer cap/casualties, the four command identities, selected-or-all dispatch, local-defend/seek/follow/flee intents, all placed pickup rewards, and the confirmed absence of defeated-enemy drops are recovered and active. Next recover exact fixed-point ray/floor selection, the remaining scheduler corner-contact branches, movement cadence for all actor modes, multi-actor destination behavior, and broader enemy-AI rules.
