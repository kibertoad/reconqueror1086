...
| M6: Original presentation | 9 | In progress: title, options hub with original state widgets, character screens, estate/map panels, all ten executable-ordered Home hotspots, Overview/War Planning backgrounds, four descriptor-driven fief-management variants, blacksmith flow, one portrait, CD audio, and presentation descriptors are active. | Remaining War Planning/JUMP behavior, exact estate tiles, remaining CSF/SMK playback, fonts, cursors, dialogue, effects, speech, and accessibility/settings. |
