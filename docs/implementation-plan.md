...
- [x] Preserve behavior-83 exits as visible scene boundaries and import adjacent behavior-35 object states as occluded billboards with collision-aware tree, barrel/meal, stool, pot, and candelabra transitions. The executable confirms the behavior-35 weapon-contact trigger at `0x55A02` and contact distance from combat-row column 3; resource ordering is Corroborated, while exact durability and timing remain Provisional.
...
1. Continue first-person combat recovery: close the last indirect campaign-scene callback, exact state-linked foreground timing, and exact destructible durability rules; then recover remaining block transitions and replace the provisional combat, AI, retainer, loot, and consequence rules.
...
The current `main` baseline builds with zero warnings under the enforced 1,000-line source limit. All 124 xUnit cases and 145 executable specifications pass. The Inno Setup 7.1.0 package compiles, installs into an isolated directory, launches through its generated **ReConqueror A.D. 1086** Start-menu shortcut, and removes that shortcut during uninstall. Repository policy passes with the expanded proprietary-image boundary.
