...
| M5: Two endings | 8 | Both routes are playable as prototypes; the decoded `TRANDRAG.SMK` approach and `DRJSTWIN.SMK` successful dragon outcome now play at their runtime transitions. | Complete Drogo/dragon quest chain and interactive dragon encounter, bind its distinct run/loss outcomes, and recover crown politics plus the original ending sequences. |
