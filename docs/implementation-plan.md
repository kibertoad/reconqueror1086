...
1. Continue first-person combat recovery: the original four render-counted blood steps are mapped to a processor-independent compatibility timer because executable analysis proves no stable original duration exists. Pointer-to-foreground provenance, crossbow lifetime, authored friendly-placement selection, campaign retainer cap/casualties, the four command identities, selected-or-all dispatch, local-defend/seek/follow/flee intents, all placed pickup rewards, and the confirmed absence of defeated-enemy drops are recovered and active. Next recover direct actor/cell selection, exact fixed-point movement/pathfinding and cadence, and broader enemy-AI rules.
