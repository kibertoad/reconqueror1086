...
1. Continue first-person combat recovery: projected surface contacts and non-cardinal actor acquisition now share the recovered fixed-point projection, decoded elevations, texture alpha, live actor identity, and authored candidate order. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic tie-breaking are active. Kind-0 Retreat now follows mode 5 through regroup mode 7, the mode-1/3/4 formation decision, renewed mode-8 pursuit, strict contact, and the first delayed mode-11 strike. Next close mode-11 outcome transition `0x4E77E`, mode 13, and broader enemy AI. Handler `0x50020` remains excluded because the complete supported population cannot enter it.
   ...
   Mode-8 contact update (2026-09-13): predicate `0x4F7A2` rays toward stored target `+0x24`, resolves the actual hit through `0x4CEA0`, and accepts any living opposite-side actor only when returned depth is strictly below raw contact column `0xCE24 + 28 * sourceRow`. Kind-0 fixup source `0x4E458` resolves transition `0x4E745`: success selects mode 11, failure mode 6. Mode 6 clears its target and returns to the already recovered `0x40` wandering/acquisition loop. Handler `0x5010B` then repeats target validation: fixed Manhattan separation `abs(dx8) + abs(dy8) <= 0x154` uses the stored actor and comparison distance `0x154`; otherwise it casts a fresh ray, accepts an intervening opponent, and still applies the strict raw-row limit. Scheduler `0x53365` invokes damage `0x4F070` only when the mode-11 effect reaches its strict completion gate. Runtime tests cover below-boundary delayed damage and exact-boundary rejection into wandering. This Retreat-derived route is Confirmed through its first strike; mode-11 transition `0x4E77E` and mode 13 are next. GameFAQs has no comparable internal detail.
