...
- [x] Preserve behavior-83 exits as visible scene boundaries and render every non-actor kind-4 pickup, debris, and behavior-bit-`0x20` contact object as an occluded billboard. The executable confirms weapon contact at `0x55A02`, combat-row column-4 distance, and one-contact replacement through the record's offset-`0x40` state target. It also confirms behavior-19 pickup dispatch through explicit action, a strict `< 0x280` range, callback invocation, and immediate state replacement at `0x55524`-`0x55744`. The behavior-19 handler is sound-only (`0x3FFEA`); the separate reward mutation remains provisional.
...
1. Continue first-person combat recovery: recover the exact randomized foreground trajectory, processor-independent presentation of the original four render-counted blood steps, and door timing; then locate behavior-19's separate reward mutation and replace the provisional combat, AI, retainer, loot, and consequence rules.
   The timing architecture, state binding, and runtime path are now mapped and active: Scenario offset `0x1C` counts `0x40`-byte `SFXDEFS` descriptors; `0x4C7F4` copies them into live records, and `0x530D0` advances their independent deadlines immediately before actor thinking and rendering. Attack/hit/death copy adjacent state blocks and use their selected descriptors; renderer `0x46E3B`-`0x46F23` selects a heading-derived image from each block rather than consuming textures sequentially. All 144 placed actor bases across 42 owned combat scenes use angular divisions `8/4/4/2`, with 80 state families at 384 ms and 64 at 400 ms; ranged attack rows 23-24 double their interval. The imported layout, simulation gate, renderer, decoder, synthetic fixtures, and owned-resource census now cover this path. Next recover door and foreground cadence, preserving original state order and deadlines without tying progress to processor throughput, render calls, monitor refresh, or adjustable wall clock.
...
The current migration branch builds with zero warnings under the enforced 1,000-line source limit. All 140 xUnit cases and 146 executable specifications pass. The Inno Setup 7.1.0 package compiles, installs into an isolated directory, launches through its generated **ReConqueror A.D. 1086** Start-menu shortcut, and removes that shortcut during uninstall. Repository policy passes with the expanded proprietary-image boundary.

First-person combat now has the original `SKIRMISH.PCX` shell, exact viewport and panel geometry, scene-driven walls and actors, animated foreground weapons and blood effects, and the generated no-media fallback. The DOS/16M nested-module mapping correction places the LE data-page base at raw file offset `0x4C254` and `Pal%d` at object 2 offset `0x7D28`; its generator, loader, and repeated render paths establish the palette association, Scenario-driven map count, distance shift, blend target, per-block offset, fixed-depth clamp, RGB blend, half-up rounding, and Manhattan nearest-color rule now used by the runtime. All 90 owned active families match regenerated output byte-for-byte. Executable intersection masks and the corresponding render selector also confirm north/east/south/west at block offsets 44/48/52/56. Weapon setup and selection confirm the complete store-item-to-combat-row permutation, Light and Heavy Crossbow rows, family foreground bases, row-specific break-roll ranges, dice, armor penetration, and symmetric contact distances now used by the runtime. Placed actors retain their scene-selected row and one of ten executable-initialized armor/health templates; all 1,002 owned combat-scene actor placements use bounded values. Explicit record-offset-`0x40` targets replace contacted objects instead of inferred adjacent states. The renderer confirms the melee offset-2/offset-1/offset-0 approach/contact/return order and the crossbow's fixed offset 2. Target-hit dispatch at `0x55171`-`0x55189` chooses the large frame-43 effect for a fatal hit and the small frame-48 effect otherwise; its renderer consumes four frames. Scene loading at `0x50AAC`-`0x50ACC` binds Scenario offset 28 to the count of `0x40`-byte `SFXDEFS` records, whose offset-`0x14` millisecond intervals feed live animation-record constructor `0x4C7F4`; the 11 owned `MELEE0.RES` values range from 100 to 250 ms. Scene-name traces keep the tournament two-digit family out of castle assaults, select all three unsuffixed scenes for practice, and use the confirmed common `MELEE0.RES` for campaign castles and London through engine callback field `+0xB0`, its sole indirect invoker, and dispatcher slot 128. Actor state copying, the renderer's angular-sector/mirror formula, all 144 placed state families, imported `SFXDEFS` completion gates, ranged attack doubling, and strict deadline behavior are now mapped and active. Combat-row column 3 supplies the signed foreground velocity formula `(screen displacement << 9) / divisor`, integrated with elapsed milliseconds rather than a frame timer; all 25 divisors are mapped. The exact continuation point is documented in `docs/handover.md`: recover randomized foreground trajectory and door timing, adapt render-counted blood safely, and locate behavior-19's separate reward mutation.
