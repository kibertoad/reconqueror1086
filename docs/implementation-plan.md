...
1. Continue first-person combat recovery: projected surface contacts and non-cardinal actor acquisition now share the recovered fixed-point projection, decoded elevations, texture alpha, live actor identity, and authored candidate order. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic tie-breaking are active. Kind-0 Retreat now follows mode 5 through regroup mode 7 and the recovered mode-1/3/4 formation decision loop into either renewed regroup, fallback mode 2, or the first direct mode-8 pursuit effect. Next close mode-8 contact transition `0x4F7A2`/`0x4E745`, its mode-11 continuation on this route, and broader enemy AI. Handler `0x50020` remains excluded because the complete supported population cannot enter it.
