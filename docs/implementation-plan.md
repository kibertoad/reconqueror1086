...
dynamic kind-0 route: mode-11 transition `0x4E77E` selects mode 13 when target
health exceeds source health and helper `0x4E5F0` cancels the new attack
effect. Predicate `0x4FD9A` tests source health `>= 6`; mode-13 transition
...
rotation. `OriginalActorMotion`, `SiegeSession`, and focused health-boundary,
cancellation, and integer-rotation tests implement this Confirmed route.
...
1. Continue first-person combat recovery: projected surface contacts and non-cardinal actor acquisition now share the recovered fixed-point projection, decoded elevations, texture alpha, live actor identity, and authored candidate order. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic tie-breaking are active. Kind-0 Retreat now follows mode 5 through regroup mode 7, the mode-1/3/4 formation decision, renewed mode-8 pursuit, strict contact, mode-11 target/source health comparison, mode-13 health-six decision, and reachable mode-10 non-cardinal escape. Continue with broader enemy AI and later formation outcomes.
   ...
   Mode-8/post-strike update (2026-09-13; completed 2026-09-14): predicate `0x4F7A2` accepts an actual opposite-side ray hit only below raw contact column `0xCE24 + 28 * sourceRow`; transition `0x4E745` selects mode 11 or failure mode 6. Handler `0x5010B` repeats validation through the `<= 0x154` Manhattan shortcut or a fresh ray, and scheduler `0x53365` applies damage only at effect completion. The subsequent comparison at `0x504F2` keeps mode 11 only when target health is `<=` source health. A stronger target selects mode 13 through `0x4E77E` and `0x4E5F0` cancels that new strike. Predicate `0x4FD9A` and transition `0x4E7A4` select mode 2 at health `>= 6`, otherwise reachable mode 10 and its 1.5-scaled non-cardinal escape. Runtime tests cover contact equality, delayed damage, cancellation, health 5/6, and exact integer movement. GameFAQs has no comparable internal detail.
