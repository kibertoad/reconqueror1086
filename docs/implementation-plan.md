...
Combat recovery update (2026-09-10): placed behavior-135 actors now decode their block-offset-`0x4A` template and `0x4C` combat row. The runtime uses the ten executable-initialized armor/health pairs, all 25 rows' dice and armor penetration, explicit object state targets, and symmetric row-specific contact distances for player and enemy attacks. This narrows the open combat work to exact hit eligibility/positional modifiers, cadence, grid/sub-cell reconciliation, movement AI, retainers, loot, and consequences rather than the already-restored damage and reach tables.
...
1. Continue first-person combat recovery: close the last indirect campaign-scene callback, exact state-linked foreground timing, and behavior-19 pickup dispatch; then replace the provisional combat, AI, retainer, loot, and consequence rules.
