...
- 132 xUnit test cases and 146 broader executable specifications passing without a graphics device through the isolated Windows test launcher.
...
- [x] Preserve behavior-83 exits as visible scene boundaries and render every non-actor kind-4 pickup, debris, and behavior-bit-`0x20` contact object as an occluded billboard. The executable confirms weapon contact at `0x55A02`, combat-row column-4 distance, and one-contact replacement through the record's offset-`0x40` state target. It also confirms behavior-19 pickup dispatch through explicit action, a strict `< 0x280` range, callback invocation, and immediate state replacement at `0x55524`-`0x55744`; callback-defined rewards remain provisional.
...
1. Continue first-person combat recovery: close the last indirect campaign-scene callback and exact state-linked foreground timing; then recover behavior-19 callback rewards and replace the provisional combat, AI, retainer, loot, and consequence rules.
   The timing architecture and resource source are now mapped: Scenario offset `0x1C` counts `0x40`-byte `SFXDEFS` descriptors; fields `+0x0C`, `+0x14`, `+0x18`, and `+0x28` carry frame count, millisecond interval, flags, and optional map-block selector, while the high word of scene-block field `+0x2C` selects a descriptor. `0x4C7F4` copies those values into live 64-byte records, `0x530D0` advances their deadlines immediately before the separate `0x50524` actor-thinker pass and world render, rows 23-24 double the selected interval, weapon poses are state/contact-selected, and blood consumes four foreground render ticks. The bounded decoder and owned-disc inspection now validate every `SFXDEFS` table; next bind descriptors to exact state events and remove each provisional universal seconds constant only when its event mapping is proven.
