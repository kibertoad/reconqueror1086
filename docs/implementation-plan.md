...
| M4: Conquest | 6-7 | Playable prototype: the manual-confirmed five-division War Planning roster, company editing, membership, field state, persistent spies, and joined-division travel/combat state are active. | Independent captain-commanded division movement, remaining original strategic rules, battlefield systems, castle layouts, combat balance, retainers, and loot. |
...
1. Continue tracing startup/menu input timing and HAT region-action dispatch from `CONQUER.EXE`; all ten Home labels and War Planning control frames are mapped, while independent division movement, exact spy timing/cost, `JUMP!!`, and original input timing remain to be confirmed.
