...
- Eighty-one xUnit test cases and 143 broader executable specifications passing without a graphics device through the isolated Windows test launcher.
...
- [x] Decode the complete canonical `Pal0`-`Pal127` scene color-map set as four 32-step indexed-remap families. Exact runtime family selection and distance/effect scaling remain unresolved, so these tables are retained but not yet applied.
