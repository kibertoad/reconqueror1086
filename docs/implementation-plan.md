...
1. Continue first-person combat recovery: close the last indirect campaign-scene callback and exact state-linked foreground timing; then recover behavior-19 callback rewards and replace the provisional combat, AI, retainer, loot, and consequence rules.
   The timing architecture is now mapped: `0x4C7F4` creates 64-byte records with per-record millisecond intervals, `0x530D0` advances their deadlines immediately before the separate `0x50524` actor-thinker pass and world render, weapon poses are state/contact-selected, and blood consumes four foreground render ticks. Next trace interval arguments back to their resource/template fields and reproduce those records; do not promote the current universal seconds constants.
...
The current `main` baseline builds with zero warnings under the enforced 1,000-line source limit. All 131 xUnit cases and 145 executable specifications pass. The Inno Setup 7.1.0 package compiles, installs into an isolated directory, launches through its generated **ReConqueror A.D. 1086** Start-menu shortcut, and removes that shortcut during uninstall. Repository policy passes with the expanded proprietary-image boundary.
