...
Current progress: the owned scene archives' bounded `Viewer`, `Scenario`, `Map`, `Blocks`, `Backdrop`, `BackImage`, and complete `Pal0`–`Pal127` color-map families now drive collision maps, initial position/facing, panorama, named doors and secret passages, food/treasure roles, defender/champion placements, and indexed wall shading in practice and campaign siege sessions. The 15 ordinary castle destinations select the correspondingly ordered `MELEE00`–`MELEE24` three-by-five scene family; exact London/base-scene selection remains open. Import floods from the Viewer and crops the 128x128 source map to its reachable play area plus enclosing wall; this excludes disconnected authoring galleries where they are separate from that component, while the defense scene currently retains its connected exterior. The palette-verified 320x200 `SKIRMISH.PCX` shell now supplies the original stone frame, combat viewport, message/status panels, command labels, health strip, and radar well. A map-aware raycast renders inside that exact 167x117 source-pixel viewport, with the generated full-canvas interface retained as the no-media fallback. It presents the facing-wrapped original panorama, contacted cardinal wall/door texture slot, and knight/champion/footman billboard runs as three walk poses across five rear-to-front viewing angles, mirrored opposite angles, nine-frame attacks, directional hit reactions, and eight-frame collapses with per-column occlusion. The 53-frame `SKIRMISH.CSF` sequence supplies original-scale, viewport-clipped axe, crossbow, hammer, mace, sword, and dagger foreground attacks plus separate large player-damage and small successful-hit blood sequences through its own raw `SKIRMISH.PAL`; frame grouping and visible identities are recovered, while exact category binding and cadence remain provisional. Dead actors cease collision and combat immediately but remain visible until their collapse finishes, delaying victory presentation appropriately. Door interaction retains collision while advancing through adjacent original intermediate/open state records, then exposes the passage. Original indexed wall pixels are remapped through the confirmed identity-to-black `Pal32`–`Pal63` family and converted once per encountered texture/shade pair; the current `Pal32 + floor(distance)` selection is deliberately isolated and Provisional. Missing media uses shaded structures and generated combatants, and the generated 12x12 keep remains the no-media fallback. Exact London archive selection, executable confirmation of cardinal slot and palette association, exact color-map family/scale selection, exact status-field contents and controls, executable confirmation of blood-effect triggers, non-door block transitions, exact door and foreground timing, AI, combat balance, retainers, and loot/consequence tables remain open.
...
| M4: Conquest | 6-7 | Playable prototype: the manual-confirmed five-division War Planning roster, company editing, membership, field state, persistent spies, joined-division combat, persisted captain-commanded movement, and original scene-map/start/spawn ingestion are active. The 15 ordinary castles now select the ordered three-by-five melee scene family. Imported first-person scenes render inside the original combat shell and viewport with cropped connected maps, panoramas, contacted and indexed-distance-shaded wall faces, animated door states, direction-aware combatants, attacks, hit reactions, complete death sequences, equipped-weapon foreground attacks, player-damage blood, and successful-hit impact effects with occlusion. | Exact route interaction and captain battle rules, remaining original strategic rules, London/base-scene selection, executable confirmation of surface/color-map, status/control, and blood-trigger rules, non-door state transitions, exact animation cadence and weapon-category binding, combat balance and AI, retainers, and loot. |
| M5: Two endings | 8 | Both routes are playable as prototypes. The dragon moor stays hidden until original global variable 2 is raised by Anna Lisa's decoded secret-lair branch; clean-room mode mirrors that disclosure after her second successful courtship joust, and older saves already at the moor migrate safely. The route then requires its items and Mighty strength before entering a timed, moving-eye, one-thrust encounter; strength affects the hit window, retreat remains possible, and the owned `DRJSTWIN.SMK`, `DRJSTLSE.SMK`, and `DRJSTRUN.SMK` sequences play for its three outcomes. The imported `DRJSTWIN.PCX` and 25-frame `lance1.CSF` supply its original background, palette, and foreground lance. | Complete absent-dragon plunder/visit consequences and the Drogo chain, replace remaining shortcut courtship rewards with decoded dialogue state, confirm exact dragon target path/timing and lance frame placement from the executable, and recover crown politics plus the original ending sequences. |
| M6: Original presentation | 9 | In progress: file-backed direct title/credits/item-movie playback, the confirmed campaign briefing, title fallback, options hub with original state widgets and persistent channel volumes/reduced motion, pause, a centered aspect-correct virtual canvas with windowed/borderless-fullscreen and integer-scaling modes, character screens, estate/map panels, all ten executable-ordered Home hotspots, Overview/War Planning backgrounds and controls, four descriptor-driven fief-management variants, the populated inn with ten named patrons and portraits, blacksmith flow, CD audio, presentation descriptors, edge-triggered controller navigation, context-sensitive dialogue/map/shop/tournament/combat actions, and a right-stick virtual pointer for all original hotspots are active. | Exact army-path/JUMP behavior, exact estate tiles, patron dialogue, remaining CSF/SMK event bindings and seeking, fonts, cursors, dialogue, subtitle controls, and input remapping. |
...
- [x] Classify solid-block cardinal surface slots separately from kind-4 billboard references and render the contacted wall or door face through the map-aware raycaster. The observed slot order and `SKIRMISH.PAL` association remain corroborated pending executable confirmation.
...
- [x] Decode the complete canonical `Pal0`-`Pal127` scene color-map set as four 32-step indexed-remap families. Apply the confirmed identity-to-black `Pal32`-`Pal63` family to original wall pixels through a bounded, retained texture/shade cache; exact runtime family selection and distance/effect scaling remain Provisional and isolated behind `SiegeColorMapping`.
- [x] Replace modulo-three campaign scene selection with the resource-correlated 15-castle mapping across `MELEE00`-`MELEE04`, `MELEE10`-`MELEE14`, and `MELEE20`-`MELEE24`. The exact London mapping to an unsuffixed base scene remains Provisional.
- [x] Decode and palette-verify the 53-frame `SKIRMISH.CSF` sequence, classify its weapon, shield, and blood-effect runs, and bind the axe, crossbow, hammer, mace, sword, dagger, large player-damage, and small successful-hit sequences to first-person combat. Exact frame cadence, equipment-category edge cases, and blood-trigger semantics remain Provisional.
...
1. Continue first-person combat recovery: trace the executable's `Pal%d` loading and selection path to establish the color-map family and distance/effect scale; confirm the four-side surface order, London/base-scene selection, foreground frame cadence, weapon-category dispatch, and blood-effect triggers; then recover non-door block transitions and replace the remaining provisional combat, AI, retainer, loot, and consequence rules.
...
The current `main` baseline builds with zero warnings under the enforced 1,000-line source limit. All 98 xUnit cases and 143 executable specifications pass. The Inno Setup 7.1.0 package compiles, installs into an isolated directory, launches through its generated **ReConqueror A.D. 1086** Start-menu shortcut, and removes that shortcut during uninstall. Repository policy passes with the expanded proprietary-image boundary.
