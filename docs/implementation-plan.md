...
1. Continue first-person combat recovery: projected surface contacts and non-cardinal actor acquisition now share the recovered fixed-point projection, decoded elevations, texture alpha, live actor identity, and authored candidate order. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic tie-breaking are active. Kind-0 Retreat now follows mode 5 through regroup mode 7, the mode-1/3/4 formation decision, renewed mode-8 pursuit, strict contact, and completion-gated mode-11 damage. Mode-14 hit handling performs the stored-target/source-health comparison before mode 13's health-six decision and reachable mode-10 non-cardinal escape. Supported hostile pursuit, attack, hit/morale, and escape states are active. Continue with hostile modes 1-3, exact no-effect thinker cadence, and later formation outcomes.
