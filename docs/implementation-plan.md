...
1. Continue first-person combat recovery: close the last indirect campaign-scene callback, exact state-linked foreground timing, and exact destructible reach/durability rules; then recover remaining block transitions and replace the provisional combat, AI, retainer, loot, and consequence rules.
