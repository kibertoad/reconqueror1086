...
- Retainer count scaling is recovered; continue with formations, commands, pathfinding, survival, and post-battle army losses.
...
Combat recovery update (2026-09-13, retainer quantity): the campaign assault caller at `0x399F6`-`0x39A22` obtains the army total through `0x2D500`, divides it by 50 with integer truncation, and clamps the result to 2-10 before passing it to setup `0x5877C`. Setup stores the value at `D49C`; loader pruning at `0x51B87` compares it with `0x4D870`, whose actor-table loop counts living friendly actors but explicitly excludes the player index `D4D0`. The resulting original formula is `retainers = clamp(floor(army total / 50), 2, 10)` (**Confirmed** for the hashed release). `OriginalRetainerCombat` now owns that mapping and replaces the prior clean-room `1 + army / 10` approximation. Formation, actor AI, survival, and the caller's later unit-type loss distribution remain open rather than being inferred from the count formula.
...
1. Continue first-person combat recovery: the original four render-counted blood steps are mapped to a processor-independent compatibility timer because executable analysis proves no stable original duration exists. Pointer-to-foreground provenance, crossbow lifetime, retainer quantity, all placed pickup rewards, and the confirmed absence of defeated-enemy drops are recovered and active. Next replace provisional retainer formation/AI/survival, siege-consequence, and broader enemy-AI rules.
