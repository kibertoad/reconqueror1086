...
- Retainer count scaling, authored actor materialization, command identities, shell hit regions, selected-or-all dispatch, and post-battle army losses are recovered; continue with formations, direct target selection, pathfinding, and survival behavior.
...
Combat recovery correction (2026-09-13, public Retreat state route): command handler `0x5744B` writes requested mode 10, resets current mode, and calls `0x4E5F0`. On the next thinker pass, acquisition `0x4F98D` scans opposite-side actors through raycaster `0x470A8`, accepts the nearest visible identity, and stores its integer coordinate snapshot at actor `+0x28/+0x2C`. The actor-kind transitions then select the next state: friendly kind-0 entry `0x4E76B` chooses previous/failure mode 2 and requested/success mode 5; friendly kind-1 entry `0x4E805` chooses mode 2 or mode 4. This **Disproves** the earlier attribution of handler `0x50020` as the public command's direct away-vector action. Kind-0 mode-5 handler `0x4FDCD` preserves the current heading and rewrites the descriptor low flags as `(flags & 0xA7) | 0x40`, leaving official `0x142` unchanged. On collision, scheduler `0x53C13`-`0x53C6C` clears each blocked coordinate remainder and sets `heading = (((heading + 0x20) & 0xC0) - 0x40) & 0xFF`, the cardinal direction to the left, without flag-`0x10` early termination. `OriginalCombatantTemplates.ActorKindForSceneTemplate`, imported spawns, and `SiegeSession.AdvanceRetainerMovement` now preserve the kind selector and run kind-0 Retreat through this processor-independent sub-cell/left-turn path; kind 1 is deliberately not sent through melee mode 5. Tests cover all ten template-kind values, imported propagation, no immediate cell jump, preserved-heading ticks, the collision turn, and bowman separation. Exact ray-visible acquisition is approximated by current nearest-opponent availability; subsequent mode-5 formation transitions and kind-1 mode-4 behavior remain **Provisional**. Handler `0x50020` itself is a separate mode-9/10 path: it derives exact heading from `current - stored target`, scales descriptor x/y deltas by the object-2 `0x7CEA` double `1.5`, and installs live flags `0x112`; the complete table/initializer census below proves it dormant for supported official gameplay. The GameFAQs guide does not describe these internal states or collision rules and therefore neither corroborates nor conflicts with the correction.

Combat recovery update (2026-09-13, authored actor sides): loader `0x51560` scans the map x-major at `0x517B7`, recognizes an actor from behavior bit `0x80` plus selector 1, takes template/row from block `+0x4A/+0x4C`, and records the first normalized side-zero actor as player `D4D0` at `0x51919`-`0x5192E`. A complete census of the supported 42 `MELEE*`/`DEFEND*` archive variants covers 144 actor-base blocks and all 1,002 placements: templates 0/1/2 are exclusively friendly, while 3/5/8/9 are exclusively hostile. That population mapping is **Confirmed** by executable control flow plus decoded official resources. `ImportedSiegeLayouts` now uses the exact actor predicate and template partition, omits the first x-major friendly as player, excludes actors from scene objects, and retains remaining friendly positions. `SiegeSession` applies `min(campaignCap, authoredRetainerCount)`, matching the loader's prune-only behavior. Raw surface/color normalization, formation, commands, and retainer movement remain open.
...
Combat recovery closure (2026-09-13, dormant modes 9/10): every kind-table
...
Handler `0x50020` is therefore **Confirmed unreachable** for supported official
gameplay; its `1.5` non-cardinal formula remains a documented dormant design
detail, not a runtime migration requirement. `OriginalCombatantTemplates.ModeProfileForSceneTemplate`
and focused tests preserve the exact table and supported-population proof. The
GameFAQs guide has no internal actor-state account and neither corroborates nor
conflicts with this executable/resource conclusion. The priority list's older
request to recover `0x50020` entry routes is superseded by this proof.
...
bridge. Next close the candidate helper's returned surface, cropped-layout
wrapping policy, and exact simultaneous-corner behavior. The trusted GameFAQs
FAQ 66730 contains no internal ray mathematics and is not corroboration here.

1. Continue first-person combat recovery: the original four render-counted blood steps are mapped to a processor-independent compatibility timer because executable analysis proves no stable original duration exists. Pointer-to-foreground provenance, direct visible actor/object identity, empty-ground mode-12 semantics and fixed-point camera/vector projection, strict imported 200 ms ticks and retained 8.8 offsets, direct cardinal re-aim, authored bit-2 blockers, live-`0x112` collision stop/retry, no-visible-target Attack mode 6, melee Retreat's preserved-heading flag-`0x40` left turn, kind-1 Attack/Retreat mode-11 completion cadence, shared-destination single-cell occupancy, and deterministic authored-order tie-breaking are recovered and active, alongside crossbow lifetime, authored friendly-placement selection, campaign retainer cap/casualties, the four command identities, selected-or-all dispatch, Defend/Attack/Follow intent, all placed pickup rewards, and the confirmed absence of defeated-enemy drops. The tie-break is an explicit compatibility adaptation because the original thinker cursor advances with an unrestricted loop and has no processor-stable exact-tie winner. Next close exact projected candidate-surface, wrapping, and corner selection, then later mode-5 formation transitions and broader enemy-AI rules. Handler `0x50020` is excluded from runtime scope because the complete supported population cannot enter it.
   Mode-6 correction (2026-09-13): public Attack requests mode 6. Acquisition-table entry `0x4F98D` searches for the nearest visible opposite-side actor; kind-0 transition `0x4E71F` and kind-1 transition `0x4E745` retain mode 6 on failure, while success selects modes 8 and 11 respectively. Current-mode handler `0x4FDCD` preserves heading, clears target fields, selects movement descriptor `+0x46`, and rewrites its low flags as `(flags & 0xA7) | 0x40` (the official `0x142` remains `0x142`). The scheduler's flag-`0x40` collision branch clears the blocked local axis and turns left with `(((heading + 0x20) & 0xC0) - 0x40) & 0xFF` while the effect continues. `SiegeSession` uses the bounded cell-visibility compatibility bridge for acquisition and stores the chosen movement family on the live effect so an order cannot alter collision semantics mid-cycle. Focused tests cover preserved-heading motion and collision continuation. The state route, flag formula, and scheduler response are Confirmed from the hashed executable; exact equivalence of the visibility bridge to raycaster `0x470A8` remains Corroborated. The GameFAQs guide contains no internal state or movement-flag detail, so it neither corroborates nor conflicts with this recovery.
   The timing architecture, state binding, and runtime path are now mapped and active: Scenario offset `0x1C` counts `0x40`-byte `SFXDEFS` descriptors; `0x4C7F4` copies them into live records, and `0x530D0` advances their independent deadlines immediately before actor thinking and rendering. Attack/hit/death copy adjacent state blocks and use their selected descriptors; renderer `0x46E3B`-`0x46F23` selects a heading-derived image from each block rather than consuming textures sequentially. All 144 placed actor bases across 42 owned combat scenes use angular divisions `8/4/4/2`, with 80 state families at 384 ms and 64 at 400 ms; ranged attack rows 23-24 double their interval. Door actions are immediate state-target replacements rather than scheduled animations. Foreground setup now follows the exact row-family random spans and coordinate branches at `0x54C07`-`0x54F12`, fixed-point motion at `0x54F98`-`0x5501E`, contact reversal at `0x550F2`-`0x55140`, and pose/mirror selection at `0x5529B`-`0x55448`. Runtime motion retains 8.8 subpixels and caps an update at the original 166 ms so processor speed and render frequency cannot affect its path. Cursor top-left plus `(9,9)` travels from `0x55DB8`-`0x55DE5` unchanged through `0x5590C`-`0x5591D`; rows 23-24 rise from viewport bottom to `bottom - sprite height`, retain offset 2, and remain held. Mouse/controller aperture clicks now supply those foreground coordinates, with keyboard centering retained as an accessibility fallback. The fixed-point empty-floor camera/vector arithmetic and 64-step bound are active; full projected-candidate output and exact corner/wrap equivalence remain provisional.
