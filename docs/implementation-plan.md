...
**Confirmed** behavior. Exact simultaneous-tie winner identity remains
**Provisional** until thinker cursor `D5C4` and effect-slot allocation are
reproduced. GameFAQs has no equivalent internal account.
This update supersedes the earlier clauses that leave all multi-actor
destination behavior Provisional; only exact simultaneous winner ordering is
still open.
...
1. Continue first-person combat recovery: the original four render-counted blood steps are mapped to a processor-independent compatibility timer because executable analysis proves no stable original duration exists. Pointer-to-foreground provenance, direct visible actor/object identity, empty-ground mode-12 semantics, strict imported 200 ms ticks and retained 8.8 offsets, direct cardinal re-aim, authored bit-2 blockers, live-`0x112` collision stop/retry, no-visible-target Attack mode 6, melee Retreat's preserved-heading flag-`0x40` left turn, kind-1 Attack/Retreat mode-11 completion cadence, and shared-destination single-cell occupancy are recovered and active, alongside crossbow lifetime, authored friendly-placement selection, campaign retainer cap/casualties, the four command identities, selected-or-all dispatch, Defend/Attack/Follow intent, all placed pickup rewards, and the confirmed absence of defeated-enemy drops. Next recover exact fixed-point ray/floor selection, later mode-5 formation transitions, exact simultaneous-tie scheduling, and broader enemy-AI rules. Handler `0x50020` is excluded from runtime scope because the complete supported population cannot enter it.
