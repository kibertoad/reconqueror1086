...
M6 correction (2026-09-22): `font.CSF` is now a required 256-frame 11-by-13 fixed ASCII source face. `OriginalUiFont` uses its alpha masks for all runtime text while preserving existing clean-room slots. Recover screen-specific font selection/advance and remaining cursor timing/hourglass behavior; the prior broad M6 “fonts, cursors” item is superseded.
