...
tests for each boundary. Exact kind-4 billboard transformation and per-pixel
actor occlusion remain Corroborated. The trusted GameFAQs FAQ 66730
contains no internal ray mathematics and is not corroboration here.

1. Continue first-person combat recovery: projected surface contacts now use fixed horizontal columns, decoded block elevations, texture alpha, selected-primary dispatch, exact multi-candidate order, diagonal/center-plane shapes, wrapped lookup, and simultaneous-corner probes; the former empty-floor return is Disproved. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct cardinal re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic authored-order tie-breaking are active. Next close exact kind-4 billboard transformation/per-pixel actor occlusion, then mode-5 formation transitions and broader enemy AI. Handler `0x50020` remains excluded because the complete supported population cannot enter it.
   ...
   The timing architecture, state binding, and runtime path are mapped and active. Scenario offset `0x1C` counts `SFXDEFS`; `0x4C7F4` copies them into live records and `0x530D0` advances independent deadlines. Actor state rendering, foreground trajectories, the 166 ms update cap, and centered pointer provenance retain their documented formulas. Fixed horizontal pointer arithmetic, block elevation spans, alpha, the 64-step bound, multi-candidate continuation, diagonal/corner geometry, and wrapped source lookup are active; exact kind-4 billboard equivalence remains provisional.
