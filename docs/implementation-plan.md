...
- Eighty-three xUnit test cases and 143 broader executable specifications passing without a graphics device through the isolated Windows test launcher.
...
| M4: Conquest | 6-7 | Playable prototype: the manual-confirmed five-division War Planning roster, company editing, membership, field state, persistent spies, joined-division combat, persisted captain-commanded movement, and original scene-map/start/spawn ingestion are active. Imported first-person scenes now render their cropped connected maps, panoramas, contacted and indexed-distance-shaded wall faces, animated door states, direction-aware combatants, attacks, hit reactions, and complete death sequences with occlusion. | Exact route interaction and captain battle rules, remaining original strategic rules, castle-to-scene selection, executable confirmation of surface/color-map rules, non-door state transitions, exact animation cadence, combat balance and AI, retainers, and loot. |
...
1. Continue first-person combat recovery: trace the executable's `Pal%d` loading and selection path to establish the color-map family and distance/effect scale; confirm the four-side surface order and exact per-location scene selection; then recover non-door block transitions and replace the remaining provisional combat, AI, retainer, loot, and consequence rules.
