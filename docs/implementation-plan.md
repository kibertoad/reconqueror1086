...
2/3 to center planes, and kinds 5/6 to opposing diagonals. `0x44F7C` retains no
more than 32 contacts. `0x470A8` projects block `+0x1C/+0x20` lower/upper
...
Runtime fixed first-solid contact, block bounds, alpha, and dispatch precedence
are active; next close transparent-candidate continuation, diagonal shapes,
cropped wrapping, and simultaneous-corner order. The trusted GameFAQs FAQ 66730
contains no internal ray mathematics and is not corroboration here.

1. Continue first-person combat recovery: projected surface contacts now use fixed horizontal columns, decoded block elevations, texture alpha, and selected-primary dispatch; the former empty-floor return is Disproved. Strict imported 200 ms movement ticks and retained 8.8 offsets, direct cardinal re-aim, authored blockers, collision families, Attack/Follow/Retreat routes, mode-11 completion cadence, single-cell occupancy, and deterministic authored-order tie-breaking are active. Next close exact multi-candidate order, diagonal/center-plane shapes, cropped wrapping, and simultaneous corners, then mode-5 formation transitions and broader enemy AI. Handler `0x50020` remains excluded because the complete supported population cannot enter it.
