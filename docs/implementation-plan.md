...
M5 correction (2026-09-22): the desktop Drogo-refusal route no longer creates the provisional 5x5 room from `DrogoEncounterDefinitions`. It requires and decodes the owned `MONEY.RES` scene, then passes its layout through `Campaign.CreateDrogoBattle(SiegeLayout)` with retainers explicitly excluded. Its confirmed scene census has one template-0 player actor and four hostile thug placements (templates 9, 8, 3, and 3), so the runtime does not preserve the earlier one-on-one fallback. This uses imported map, collision, actor, backdrop, color-map, and texture data. `MONEY.RES` as the original moneylender combat scene remains Corroborated from its owned naming/content and this binding; recover the executable dispatch before treating the association as Confirmed. Annual recurrence remains Provisional.
