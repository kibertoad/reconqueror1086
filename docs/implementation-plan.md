...
M5 correction (2026-09-22): the desktop Drogo-refusal route no longer creates the provisional 5x5 room from `DrogoEncounterDefinitions`. It requires and decodes the owned `MONEY.RES` scene, then passes its layout through `Campaign.CreateDrogoBattle(SiegeLayout)` with retainers explicitly excluded. This preserves the confirmed one-on-one campaign contract while using imported map, collision, backdrop, color-map, and texture data. `MONEY.RES` as the original moneylender combat scene remains Corroborated from its owned naming/content and this binding; recover the executable dispatch before treating the association as Confirmed. Enemy strength and annual recurrence remain Provisional.
