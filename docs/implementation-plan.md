...
- Eighty-four xUnit test cases and 143 broader executable specifications passing without a graphics device through the isolated Windows test launcher.
...
1. Continue first-person combat recovery: trace the executable's `Pal%d` loading and selection path to establish the color-map family and distance/effect scale; confirm the four-side surface order and London/base-scene selection; then recover non-door block transitions and replace the remaining provisional combat, AI, retainer, loot, and consequence rules.
