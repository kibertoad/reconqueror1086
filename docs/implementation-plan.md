...
Activation is deliberately still pending. Caller `0x3C290` orders spy reporting, player update `0x13168`, then the enemy scheduler. The player table at object-2 `+0x1A4B8` has six `0x118`-byte records: five army targets and a separate avatar record. Recover that update's movement/collision and persistence semantics next; do not synthesize these records from named campaign destinations. Then wire the fixed update, alert/encounter presentation, schema-2 migration, and dated-adapter removal as one integration batch.
