...
The owned release contains a 35,361,714-byte `.RES`-signature `C1086.GOB` archive, a raw MODE1/2352 CD data track, thousands of Smacker (`.SMK`) sequences, Sierra `.RES/.LOW` resources, and five CD-audio tracks. `tools/Conqueror.Inspect` inventories that data track; `tools/Conqueror.Import` installs owned resources locally with a provenance manifest and lossless CDDA WAV conversion. Generated proprietary files remain ignored and are not redistributed. The bounded kind-1 archive decoder is validated across all 32,089 GOB and scene blocks, while the executable-confirmed kind-2 LZW decoder validates all five of its GOB entries and all four resulting 640x480 PCX images. All 12,982 scene `TEX` entries validate as headerless byte planes whose names encode their exact dimensions, though palettes and surface mappings remain incomplete. The bounded conversation decoder validates all 1,311 `ALL.CIF`/`ALL.CBF` nodes, 2,062 prompt variants, 2,696 response branches, and 63 zero-response continuation slots; the ten populated-inn selector roots resolve to their matching named portrait nodes. The bounded SMK implementation validates and decodes all 2,131 movie headers, 182,360 indexed-video frames, and 161,884 packed-audio packets directly; the original title, credits, and `WEAPONS.DAT` item movies are now bound to a file-backed MonoGame texture/audio adapter with timing, skip, end, and return handling. All 26 `.666` sound banks decode into 102 bounded rate-tagged unsigned 8-bit mono samples. At startup every installed bank is decoded once and every sample is converted once to a retained signed 16-bit session buffer; the shared interface sample is bound to mouse activation when effects are enabled, while other event bindings remain incomplete. The runtime currently plays imported CD audio and renders the decoded original screens, movies, screenshot-confirmed one-time `FLUFF.PCX` campaign briefing, estate tiles, cursor, overlays, populated inn, and selected portraits. All ten Home hotspot labels are bound in executable order, the original Overview and War Planning backgrounds are active, and the inn uses its ten exact descriptor-defined patron actions, executable-ordered names, matching portraits, Exit action, and footer; exact army path interaction, exact spy timing/cost, JUMP behavior, dialogue conditions/mutations and runtime graph traversal, remaining SMK event bindings and seeking, broader palette associations, village/location mapping, and other Sierra payload formats remain incomplete.
