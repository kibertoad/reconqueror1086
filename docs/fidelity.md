...
The owned release contains a 35,361,714-byte `.RES`-signature `C1086.GOB` archive, a raw MODE1/2352 CD data track, thousands of Smacker (`.SMK`) sequences, Sierra `.RES/.LOW` resources, and five CD-audio tracks. `tools/Conqueror.Inspect` inventories that data track; `tools/Conqueror.Import` installs owned resources locally with a provenance manifest and lossless CDDA WAV conversion. Generated proprietary files remain ignored and are not redistributed. The bounded kind-1 archive decoder is validated across all 20,381 GOB and scene blocks, while the executable-confirmed kind-2 LZW decoder validates all five of its GOB entries and all four resulting 640x480 PCX images. All 26 `.666` sound banks decode into 102 bounded rate-tagged samples; PCM encoding and event bindings are not yet active. The runtime currently plays imported CD audio; renders the decoded original title, options, character, dilemma, estate/travel shell, England map, load, castle-office, farm-management, blacksmith-workshop/conversation/store, and portrait art; uses the verified options-screen palette for `OPTION.CSF` ON/OFF and Resume widgets; and uses the morality-screen palette for dilemma CSF choices. The estate tile field is provisional, and inaccurate Home hotspot guesses are deliberately disabled. SMK, broader palette-to-animation association, village/location mapping, full dialogue data, and other Sierra payload formats remain incomplete.
