...
The owned release contains a 35,361,714-byte `.RES`-signature `C1086.GOB` archive, a raw MODE1/2352 CD data track, thousands of Smacker (`.SMK`) sequences, Sierra `.RES/.LOW` resources, and five CD-audio tracks. `tools/Conqueror.Inspect` inventories that data track; `tools/Conqueror.Import` installs owned resources locally with a provenance manifest and lossless CDDA WAV conversion. Generated proprietary files remain ignored and are not redistributed. The bounded kind-1 archive decoder is validated across all 32,089 GOB and scene blocks, while the executable-confirmed kind-2 LZW decoder validates all five of its GOB entries and all four resulting 640x480 PCX images. All 12,982 scene `TEX` entries validate as headerless byte planes whose names encode their exact dimensions, though palettes and surface mappings remain incomplete. The bounded conversation decoder validates all 1,311 `ALL.CIF`/`ALL.CBF` nodes, 2,062 prompt variants, 2,696 response branches, 63 zero-response continuation slots, 423 node-action references, and 2,318 response-action references; the ten populated-inn selector roots resolve to their matching named portrait nodes. Its bounded action-tree companion validates all 689 `ALL.TMI` groups and the 2,267 actions, 10,373 expressions, and 13,352 values reachable in `ALL.TMB`; function meanings and runtime effects are not yet implemented. The bounded SMK implementation validates and decodes all 2,131 movie headers, 182,360 indexed-video frames, and 161,884 packed-audio packets directly; the original title, credits, and `WEAPONS.DAT` item movies are now bound to a file-backed MonoGame texture/audio adapter with timing, skip, end, and return handling. All 26 `.666` sound banks decode into 102 bounded rate-tagged unsigned 8-bit mono samples. At startup every installed bank is decoded once and every sample is converted once to a retained signed 16-bit session buffer; the shared interface sample is bound to mouse activation when effects are enabled, while other event bindings remain incomplete. The runtime also decodes the complete conversation database and every referenced portrait once at startup. It enters the inn's confirmed patron roots, selects original prompt variants, draws original speakers/prompts/responses, accepts recovered mouse rows and number keys, follows graph targets, times zero-choice continuations, and terminates on target zero. Exact army path interaction, exact spy timing/cost, JUMP behavior, conversation action effects and redirects, remaining conversation entry points, SMK event bindings and seeking, broader palette associations, village/location mapping, and other Sierra payload formats remain incomplete.
