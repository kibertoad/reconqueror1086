...
| `FFMOUSE.CSF` contains six 20x20 interface cursor frames; frame 0 is the default sword pointer and the remaining frames visibly represent wait/alternate interaction states. | Complete bounded decode and palette-rendered frame sheet. | Frame structure/default pointer Confirmed; palette Corroborated; alternate-frame dispatch Provisional |
...
The runtime additionally activates validated startup and character art, installed HAT geometry, `OPTION.CSF` widgets, the five-slot load screen, and the default `FFMOUSE.CSF` sword pointer. The estate view uses `ICONTEMP.PCX`, typed `ICONMAP.HAT` panels, and locally imported seasonal `ICA`/`ICS`/`ICW` tiles; exact terrain data, frame assignments, and alternate cursor dispatch remain provisional. Home/farm and blacksmith scenes use their distinct original layers, with the inventory driven by `WEAPONS.DAT`, `SWORDS.CSF`, and `BUYSELL.CSF`. Imported dilemmas use the verified selection and outcome rules while keeping original prose local. Built-in summaries remain the no-media fallback.
