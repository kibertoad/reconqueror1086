...
| All 26 `.666` resources are rate-tagged sample banks: magic `0x004A5031`, then repeated `UINT32LE` byte length, `UINT32LE` sample rate, and payload. | A bounded decoder consumes all 26 GOB/scene banks exactly: 102 samples at 11,025, 11,050, or 22,050 Hz with zero rejects. `VSMITH.666` contains two audio samples, disproving the earlier dialogue-text hypothesis. | Framing/rates Confirmed; unsigned 8-bit mono PCM interpretation Corroborated |
...
| `FCASTLE.HAT` regions cannot be assigned Home labels by using the adjacent executable string order. | That simple mapping was implemented and immediately contradicted by controlled original-game observation. | Disproved; mappings remain intentionally disabled |
