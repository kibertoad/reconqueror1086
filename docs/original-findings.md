...
| Home, farm, and blacksmith use distinct screen layers. `FOPTS.HAT` is the actual `TACTICAL.PCX` Home descriptor; despite its name, `FCASTLE.HAT` describes screen 19's `FIEFMGMT.PCX` table. Home regions 0-5 and 8 align with the visible overview book, castle model, three Farm/Village/Forest ledgers, Orders scroll, and wall map. | Filename-preserving HAT report, exact decoded images, executable label adjacency, and owner-supplied screenshot. | Background/descriptor Confirmed; seven visible-object action bindings Corroborated |
