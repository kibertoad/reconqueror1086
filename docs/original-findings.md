...
| The first field distinguishes at least stored (`0`), kind-1 (`1`), and kind-2 (`2`) entries. | The hashed GOB contains 10 equal-size kind-0 entries, 471 unequal-size kind-1 entries, and five unequal-size kind-2 entries. | Confirmed classification for this build; algorithm names remain unknown |
...
The hashed `C1086.GOB` contains 486 directory entries, 10 of which have equal stored and expanded sizes. Across the GOB and 50 imported scene archives, the installer extracts both byte-stored and kind-1 resources, while keeping the five unresolved kind-2 GOB entries inside their owning archive. A full import of the hashed installation now produces 16,092 manifest entries. Extensions remain hints rather than proof; known formats are validated before runtime decoding.
...
| `LOADGAME.PCX` is a distinct five-slot Load Game screen with a Resume control. | Exact decoded resource and local pixel inspection. | Confirmed visible structure; runtime save-slot behavior remains incomplete |
| `OPTION.CSF` contains five small frames representing ON/OFF/settings widgets, not the primary title menu. | Exact CSF decoding and local palette-assisted preview. | Corroborated visual interpretation; palette binding remains Provisional |

### Runtime coverage

The runtime additionally activates validated `char_ops.pcx` and `pregen.pcx` for the character flow, reads their installed HAT descriptors for exact clickable geometry, and registers `loadgame.pcx` for the upcoming multi-slot load screen. Region-action semantics remain corroborated rather than executable-confirmed.

“Installed” means the importer safely made owned resources available under ignored local storage. The runtime now uses validated `fftitle.pcx`, `char_ops.pcx`, `pregen.pcx`, `engmap1.pcx`, and `richard.pcc` images for the title, character flow, strategic map, and Richard's tournament-opponent screen; `loadgame.pcx` is registered but its multi-slot behavior is not implemented yet. Existing users must rerun `Install Original Resources.bat` once so manifests created before kind-1 support gain the newly decoded files. SMK playback, palette association, dialogue containers, and kind 2 remain incomplete.
