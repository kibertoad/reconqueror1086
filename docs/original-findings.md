...
| `CHARGEN.HAT` identifies `MORALITY.PCX` as screen 3 and defines three enabled 100×150 regions at `(62,310)`, `(268,310)`, and `(478,310)`. The dilemma sequences are also 99×149, but their palette and exact frame-to-region behavior remain unresolved. | Bounded HAT/PCX/CSF decoding from the hashed archive. | Confirmed background, regions, and dimensions; CSF presentation semantics Provisional |
...
The runtime additionally activates validated `char_ops.pcx`, `pregen.pcx`, and `morality.pcx` for the character flow, reads their installed HAT descriptors for exact clickable geometry, and presents `loadgame.pcx` as a five-slot load screen with bounded artwork-derived rows and Resume control. Imported mode selects the original age-specific dilemmas, resolves their verified breakpoint bands, applies typed attribute changes, and displays local outcome prose. The selected definition persists in modern saves so reloading cannot reroll it. Built-in summaries remain the no-media fallback. Region-action semantics remain corroborated rather than executable-confirmed.
