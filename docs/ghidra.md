...
For mode-12 routing, disassemble `0x445C4`, `0x4FF53`, and `0x530D0`. The first is the piecewise integer heading helper, the second applies cardinal quantization and creates the movement effect, and the scheduler span through `0x53D5F` contains the strict movement bands, behavior-bit-2 neighbor test, flag-`0x40` left turn, and return to actor thinking. Corroborate the blocker predicate by grouping `Behavior & 2` and `Placed` from `scene-block-report.txt`, then join actor `Field40` state targets back to rows in the same archive to identify the underlying block restored after movement. Generated disassembly and census rows remain ignored owner-local evidence.
