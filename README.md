...
| Combat parity | Exact click-selected world-object targeting, critical-hit behavior, sub-cell movement, exact actor pathfinding/cadence, stair transitions, enemy food use, and some presentation sequencing remain provisional. Authored combat actors use the executable's actor predicate and x-major first-friendly player rule; the supported official scene population confirms templates 0-2 as friendly and 3/5/8/9 as hostile. Retainers are limited by both those authored placements and the executable's independently capped three-unit-type formula, and dead retainers remove the represented strategic soldiers in original order. The four numeric command modes, shell hit regions, selected-or-all fallback, and their local-defend/seek/follow/flee intents are executable-confirmed and active; movement currently uses documented grid-level pathing. Executable analysis confirms that defeated enemies do not generate loot; authored scene pickups are the reward source. Placed-scene pickup rewards, melee and crossbow foreground trajectories, pointer-derived foreground placement, and door state targets follow executable-confirmed paths. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
