...
| Strategic foundation | New campaigns create and save the original-shaped property/person/movement state, decoded `icon.jp` grid, camera, terrain profile, routes, mutations, and movement remainder. Core implements recovered route, terrain, profile, and movement kernels. When that state is present, terrain uses the source tile order and clipped seasonal `ics`/`ica`/`icw` atlas. |
