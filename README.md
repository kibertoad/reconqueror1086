...
| Combat parity | Exact fixed-point ground selection, multi-actor routing, Retreat/no-target wandering, acquisition visibility, critical-hit behavior, stair transitions, enemy food use, and some presentation sequencing remain provisional. Pointer clicks preserve the exact visible actor or actionable object selected by the player: friendly clicks toggle retainer selection, hostile clicks retarget selected retainers or attack that hostile, object clicks use the original `< 0x280` action limit, and lower-viewport ground clicks send selected retainers to the resulting visible floor cell through recovered mode 12. Selected ground travel, acquired-target Attack pursuit, and Follow retain the original signed sub-cell position, three-frame walk stride, strict 200 ms movement ticks, and processor-independent rendering/picking position. They aim directly at their current target at each effect boundary and use authored collision flags instead of visual labels. A blocked step keeps its partial sub-cell position and retries on the next scheduler update; it does not turn left. The first zero-offset cell transition occurs after the 600 ms effect cycle; retained remainder makes sustained travel one cell per four ticks (800 ms), correcting the earlier universal-600-ms simplification. The runtime picker's floor-plane mapping remains a documented compatibility implementation pending byte-for-byte equivalence with the original fixed-point raycaster. Authored combat actors use the executable's actor predicate and x-major first-friendly player rule; templates 0-2 are friendly and 3/5/8/9 hostile. Retainers obey the recovered cap/loss formula and four command intents. Executable analysis also confirms that defeated enemies do not generate loot; authored scene pickups are the reward source. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
