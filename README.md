...
| Combat parity | Exact click-selected world-object targeting, critical-hit behavior, sub-cell movement, enemy/retainer AI, stair transitions, enemy food use, pickup rewards, loot, siege consequences, and some presentation sequencing remain provisional. Melee and crossbow foreground trajectories, pointer-derived foreground placement, and door state targets now follow executable-confirmed paths. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
