...
| Combat parity | Exact click-selected world-object targeting, critical-hit behavior, sub-cell movement, enemy/retainer AI, stair transitions, enemy food use, siege consequences, and some presentation sequencing remain provisional. Retainer quantity now follows the executable's army-size formula. Executable analysis confirms that defeated enemies do not generate loot; authored scene pickups are the reward source. Placed-scene pickup rewards, melee and crossbow foreground trajectories, pointer-derived foreground placement, and door state targets now follow executable-confirmed paths. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
