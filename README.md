...
| Field and siege combat | Playable formation orders and counter-based field battles. First-person assaults include imported scene geometry, facing, doors, secret rooms, radar, melee, crossbows, ammunition, champions, clickable authored actors and objects, and the original command intents: Attack seeks defenders, Defend holds position and reacts locally, Follow returns to the player, and Retreat moves away from danger. Executable-confirmed rules drive pointer action dispatch, the campaign retainer cap and resulting army losses, combat reach, hit eligibility, damage, armor penetration, combatant attributes, player health, and the exact placed-scene rewards for coins, meals, chain hauberks, and bolt piles. |
...
| Combat parity | Exact ground/cell click orders, critical-hit behavior, sub-cell movement, exact actor pathfinding/cadence, stair transitions, enemy food use, and some presentation sequencing remain provisional. Pointer clicks now preserve the exact visible actor or actionable object selected by the player: friendly clicks toggle retainer selection, hostile clicks retarget selected retainers or attack that hostile, and object clicks use the original `< 0x280` action limit. The runtime picker follows its renderer's wall depth, billboard dimensions, draw order, and texture alpha; byte-for-byte equivalence with the original fixed-point raycaster remains open. Authored combat actors use the executable's actor predicate and x-major first-friendly player rule; templates 0-2 are friendly and 3/5/8/9 hostile. Retainers obey the recovered cap/loss formula and four command intents. Executable analysis also confirms that defeated enemies do not generate loot; authored scene pickups are the reward source. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
