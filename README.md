...
| Combat parity | Exact fixed-point ground selection, multi-actor routing, Attack's no-target wandering, acquisition visibility, bowman Retreat continuation, critical-hit behavior, stair transitions, enemy food use, and some presentation sequencing remain provisional. Pointer clicks preserve the exact visible actor or actionable object selected by the player: friendly clicks toggle retainer selection, hostile clicks retarget selected retainers or attack that hostile, object clicks use the original `< 0x280` action limit, and lower-viewport ground clicks send selected retainers to the resulting visible floor cell through recovered mode 12. Selected ground travel, acquired-target Attack pursuit, Follow, and melee Retreat retain the original signed sub-cell position, three-frame walk stride, strict 200 ms movement ticks, and processor-independent rendering/picking position. Direct travel aims at its target at effect boundaries; melee Retreat preserves its current heading and, when blocked, turns left from the nearest cardinal while continuing the effect. A blocked direct step instead keeps its partial sub-cell position and retries on the next scheduler update. The first zero-offset cell transition occurs after the 600 ms effect cycle; retained remainder makes sustained travel one cell per four ticks (800 ms), correcting the earlier universal-600-ms simplification. The runtime picker's floor-plane mapping remains a documented compatibility implementation pending byte-for-byte equivalence with the original fixed-point raycaster. Authored combat actors use the executable's actor predicate and x-major first-friendly player rule; templates 0-2 are friendly and 3/5/8/9 hostile. Retainers obey the recovered cap/loss formula and four command identities. Executable analysis also confirms that defeated enemies do not generate loot; authored scene pickups are the reward source. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
