...
The importer also supports `--verify [UserContent path]` to hash-check every manifest entry and `--repair [original installation] [UserContent path]` to rebuild and verify generated content from the read-only source installation. Verification rejects missing, modified, duplicated, malformed, or root-escaping manifest entries.
