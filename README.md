...
| Field and siege combat | Playable formation orders and counter-based field battles. First-person assaults include imported scene geometry, facing, doors, secret rooms, radar, melee, crossbows, ammunition, champions, clickable authored actors and objects, selected-retainer ground orders, and the original command intents: Attack seeks defenders, Defend holds position and reacts locally, Follow returns to the player, and Retreat breaks melee retainers into their preserved-heading run state. Attack pursuit, Follow, and melee Retreat move continuously through the original sub-cell walk cadence instead of jumping by whole map cells. Executable-confirmed rules drive pointer action dispatch, the campaign retainer cap and resulting army losses, combat reach, hit eligibility, damage, armor penetration, combatant attributes, player health, and the exact placed-scene rewards for coins, meals, chain hauberks, and bolt piles. |
...
| Combat parity | Byte-identical non-cardinal acquisition rays, critical-hit behavior, stair transitions, enemy food use, and some presentation sequencing remain provisional. Pointer clicks now use the executable's ordered projected candidates rather than an invented floor plane: fixed horizontal rays, wrapped lookup, corner/perpendicular probe order, pass-through/state continuation, center and diagonal shapes, decoded elevations, wall depth, and texture alpha are active. Kind-4 actors and objects use their live centered 8.8 position, forward depth, ray-relative texture coordinate, heading sector, mirror rule, and per-pixel alpha in that same insertion order. Selected retainers consume the primary surface contact before actor/object actions as in the executable. Selected contact travel, acquired-target Attack pursuit, Follow, and melee Retreat retain the original signed sub-cell position, three-frame walk stride, strict 200 ms movement ticks, and processor-independent rendering/picking position. An Attack order with no visible target now retains the actor's heading and uses the original mode-6 wandering collision family; a blocked step clears its local remainder and turns left while the movement effect continues. Direct travel aims at its target at effect boundaries; melee Retreat uses that same preserved-heading collision response. Bowman Retreat follows the recovered mode-4-to-mode-11 route, firing at the nearest visible defender inside its original raw combat-row range instead of borrowing melee movement. A blocked direct step keeps its partial sub-cell position and retries on the next scheduler update. The first zero-offset cell transition occurs after the 600 ms effect cycle; retained remainder makes sustained travel one cell per four ticks (800 ms), correcting the earlier universal-600-ms simplification. Authored combat actors use the executable's actor predicate and x-major first-friendly player rule; templates 0-2 are friendly and 3/5/8/9 hostile. Retainers obey the recovered cap/loss formula and four command identities. Executable analysis also confirms that defeated enemies do not generate loot; authored scene pickups are the reward source. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
