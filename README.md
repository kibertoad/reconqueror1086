...
| Combat parity | Foreground input targeting, crossbow lifetime, critical-hit behavior, sub-cell movement, enemy/retainer AI, stair transitions, enemy food use, pickup rewards, loot, siege consequences, and some presentation sequencing remain provisional. Melee foreground trajectories and door state targets now follow the executable-confirmed paths. Blood keeps the original four displayed frames through a processor-independent compatibility timer because the original advanced once per unrestricted render loop. |
