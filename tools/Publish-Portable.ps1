...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path $artifactsRoot "ReConqueror1086-$Runtime"
}
...
& dotnet publish (Join-Path $repositoryRoot 'src/Conqueror.Game/Conqueror.Game.csproj') @common `
    --output $gameOutput
...
$gameExecutable = Join-Path $gameOutput 'Conqueror.Game'
$importerExecutable = Join-Path $toolOutput 'Conqueror.Import'
if (-not [IO.File]::Exists($gameExecutable)) {
    throw "Packaged game is missing at '$gameExecutable'."
}
if (-not [IO.File]::Exists($importerExecutable)) {
    throw "Packaged importer is missing at '$importerExecutable'."
}
& chmod 755 $gameExecutable $importerExecutable
